...
La regresión lineal puede considerarse un algoritmo de _machine learning_. En la Sección \@ref(two-or-seven) demostramos cómo la regresión lineal puede ser demasiada rígida para ser útil. Esto es generalmente cierto, pero para algunos desafíos funciona bastante bien. También sirve como [fix baseline approach enfoque de partida/inicial check sent] un enfoque de línea de base: si no puede mejorarlo con un enfoque más complejo, probablemente querrá continuar con la regresión lineal. [fix check eng] Para establecer rápidamente la conexión entre la regresión y el _machine learning_, reformularemos el estudio de Galton con alturas, un resultado continuo.
...
Supongamos que tiene la tarea de construir un algoritmo de _machine learning_ que prediga la altura del hijo $Y$ usando la altura del padre $X$. Generemos sets de evaluación y entrenamiento:
...
[fix 'our squared loss is' ] Nuestra pérdida al cuadrado es:

```{r}
mean((m - test_set$son)^2)
```

¿Podemos mejorar? En el capítulo de regresión, aprendimos que si el par $(X,Y)$ [fix update c.s.d] sigue una distribución normal bivariante, la expectativa condicional (lo que queremos estimar) es equivalente a la línea de regresión:
...
La función `predict` es muy útil para aplicaciones de _machine learning_. Esta función toma un objeto ajustado de funciones como `lm` o `glm` (aprenderemos sobre `glm` pronto) y un _data frame_ con los nuevos predictores para los cuales predecir. Entonces, en nuestro ejemplo actual, usaríamos `predict` así:
...
[fix check ok ] Use el paquete __caret__ para dividir/particionar en un set de evaluación y uno de entrenamiento del mismo tamaño. Entrene un modelo lineal e indique el RMSE. Repita este ejercicio 100 veces y haga un histograma de los RMSE e indique el promedio y la desviación estándar.[fix] Sugerencia: adapte el código mostrado anteriormente así:
...
2\. Ahora repetiremos lo anterior pero usando sets de datos más grandes. Repita el ejercicio 1 pero para sets de datos con `n <- c(100, 500, 1000, 5000, 10000)`. Guarde el promedio y la desviación estándar de RMSE de las 100 repeticiones. Sugerencia: use las funciones `sapply` o `map`.
...
b. Debido a la ley de los grandes números, el RMSE disminuye: más datos, estimados más precisos.
...
c. Cuando aumentamos la correlación entre `x` y `y`, `x` tiene más poder predictivo y, por lo tanto, proporciona un mejor estimado de `y`. Esta correlación tiene un efecto mucho mayor en RMSE que `n`. `n` grande simplemente provee estimados más precisos de los coeficientes del modelo lineal.
...
El enfoque de regresión puede extenderse a datos categóricos. En esta sección, primero ilustramos cómo, para datos binarios, simplemente se puede asignar valores numéricos de 0 y 1 a los resultados $y$ y aplicar la regresión como si los datos fueran continuos. Luego señalaremos una limitación con este enfoque e introduciremos _regresión logística_ como una solución. La regresión logística es un caso específico de un set de _modelos lineales generalizados_. Para ilustrar la regresión logística, [fix ok?] la aplicaremos a nuestro ejemplo anterior de predicción de sexo según altura:
...
Dado que los resultados del gráfico anterior [fix] parecen cercano a  lineal/casi lineal, y es el único enfoque que conocemos actualmente, intentaremos la regresión. Asumimos que:
...
Para formar una predicción, definimos una _regla de decisión_: predecir hembra si $\hat{p}(x) > 0.5$. Podemos comparar nuestras predicciones con los resultados usando:
...
La función $\beta_0 + \beta_1 x$ puede tomar cualquier valor, incluyendo negativos y valores mayores que 1. De hecho, el estimado $\hat{p}(x)$ calculado en la sección de regresión lineal se vuelve negativo en alrededor de 76 pulgadas.
...
La idea de los modelos lineales generalizados (_generalized linear models_ o GLM por sus siglas en inglés) es 1) definir una distribución de $Y$ que es consistente con sus posibles resultados y
2) encontrar una función $g$ tal que  $g(\mbox{Pr}( Y = 1 \mid X = x))$ se puede modelar como una combinación lineal de predictores.
La regresión logística es el GLM más utilizado. Es una extensión de regresión lineal que asegura que el estimado de $\mbox{Pr}( Y = 1 \mid X = x)$ está entre 0 y 1. Este enfoque utiliza la transformación _logística_ introducida en la Sección \@ref(logit):

$$ g(p) = \log \frac{p}{1-p}$$

[fix] Esta transformación logística convierte probabilidad en riesgo relativo. Como se discutió en [fix lecture??] la conferencia de visualización de datos, las probabilidades nos dicen cuánto más probable es que algo suceda en comparación con no suceder. $p=0.5$ significa que las probabilidades son de 1 a 1, por lo tanto, las probabilidades son 1. Si $p=0.75$, las probabilidades son de 3 a 1. Una buena característica de esta transformación es que convierte las probabilidades en simétricas alrededor de 0. Aquí hay un gráfico de $g(p)$ versus $p$:
...
Con este modelo, ya no podemos usar mínimos cuadrados. En su lugar, calculamos el _estimado de probabilidad máxima_ (_maximum likelihood estimation_ o MLE por sus siglas en inglés). Pueden obtener más información sobre este concepto en un libro de texto de teoría estadística^[http://www.amazon.com/Mathematical-Statistics-Analysis-Available-Enhanced/dp/0534399428].

[fix odd sent] En R, podemos ajustar el modelo de regresión logística con la función `glm`: modelos lineales generalizados. Esta función es más general que la regresión logística, por lo que debemos especificar el modelo que queremos a través del parámetro `family`:
...
Cuando usamos `predict` con un pbjeto `glm`, tenemos que especificar que queremos `type="response"` si queremos las probabilidades condicionales, ya que el valor por defecto es devolver [fix check this term] los valores transformados logísticos.
...
Esto no es sorprendente, dado que el estimado de $\hat{p}(x_1, x_2)$ se ve similar también:
...
Al igual que la regresión, [fix] la regla de decisión es una línea, un hecho que puede corroborarse matemáticamente desde:
...
Tenga en cuenta que hemos definido una variable `x` que es predictivo de un resultado binario `y`.
...
2\. Repita la simulación del ejercicio 1 100 veces y compare la exactitud promedio para cada método y observe como dan prácticamente la misma respuesta.
