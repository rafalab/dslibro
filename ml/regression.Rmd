...
Supongamos que tienen la tarea de construir un algoritmo de _machine learning_ que prediga la altura del hijo $Y$ usando la altura del padre $X$. Generemos sets de evaluación y entrenamiento:
...
La función `predict` es muy útil para aplicaciones de _machine learning_. Esta función toma el resultado de funciones que ajustan modelos como `lm` o `glm` (aprenderemos sobre `glm` pronto) y un _data frame_ con los nuevos predictores para los cuales predecir. Entonces, en nuestro ejemplo actual, usaríamos `predict` así:
...
Use el paquete __caret__ para dividir en un set de evaluación y uno de entrenamiento del mismo tamaño. Entrene un modelo lineal e indique el RMSE. Repita este ejercicio 100 veces y haga un histograma de los RMSE e indique el promedio y la desviación estándar. [fix como evitamos doble ":" eng too] Sugerencia: adapte el código mostrado anteriormente así
...
c. Cuando aumentamos la correlación entre `x` y `y`, `x` tiene más poder predictivo y, por lo tanto, provee un mejor estimado de `y`. Esta correlación tiene un efecto mucho mayor en RMSE que `n`. `n` grande simplemente ofrece estimados más precisos de los coeficientes del modelo lineal.
...
Con este modelo, ya no podemos usar mínimos cuadrados. En su lugar, calculamos  el _estimado de máxima verosimilitud_ (_maximum likelihood estimation_ o MLE por sus siglas en inglés). Pueden obtener más información sobre este [fix quieres poner titulo?] concepto en un libro de texto de teoría estadística^[http://www.amazon.com/Mathematical-Statistics-Analysis-Available-Enhanced/dp/0534399428].

[fix is this how you want to present? check with alex] En R, podemos ajustar el modelo de regresión logística con la función `glm`: modelos lineales generalizados. Esta función puede ajustar varios modelos, no solo regresión logística, por lo que debemos especificar el modelo que queremos a través del parámetro `family`:
...
Las predicciones resultantes son similares. Esto se debe a que los dos estimados de $p(x)$ son mayores que 1/2 en aproximadamente la misma región de x:
...
Las regresiones lineales y logísticas proporcionan un estimado de la expectativa condicional:
...
En esta sección aplicamos la regresión logística a  los datos "dos o siete" introducidos en la Sección \@ref(two-or-seven). En este caso, estamos interesados en estimar una probabilidad condicional que depende de dos variables. El modelo de regresión logística estándar en este caso supondrá que:
