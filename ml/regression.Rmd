...
[fix 'our squared loss is' alex look up in machine learnig book] Nuestra pérdida al cuadrado es:
...
El enfoque de regresión puede extenderse a datos categóricos. En esta sección, primero ilustramos cómo, para datos binarios, simplemente se puede asignar valores numéricos de 0 y 1 a los resultados $y$ y aplicar la regresión como si los datos fueran continuos. Luego señalaremos una limitación con este enfoque e introduciremos _regresión logística_ como una solución. La regresión logística es un caso específico de un set de _modelos lineales generalizados_. Para ilustrar la regresión logística, la aplicaremos a nuestro ejemplo anterior de predicción de sexo basado en altura:
...
Dado que los resultados del gráfico anterior son casi lineal, y es el único enfoque que conocemos actualmente, intentaremos la regresión lineal. Asumimos que:
...
En R, podemos ajustar el modelo de regresión logística con la función `glm`: modelos lineales generalizados (generalized linear models en inglés). Esta función puede ajustar varios modelos, no solo regresión logística, por lo que debemos especificar el modelo que queremos a través del parámetro `family`:
...
Cuando usamos `predict` con un objeto `glm`, tenemos que especificar que queremos `type="response"` si queremos las probabilidades condicionales, ya que el valor por defecto es devolver `term` que da los valores transformados logísticos.
...
Al igual que con regresión lineal, la regla de decisión es una línea, un hecho que puede corroborarse matemáticamente ya que:
