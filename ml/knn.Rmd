...
Introdujimos el algoritmo kNN en la Sección \@ref(knn-cv-intro) y demostramos cómo usamos la validación cruzada para elegir $k$ en la [fix ok] Sección \@ref(caret-cv). Aquí revisamos rápidamente cómo ajustamos un modelo kNN usando el paquete __caret__. En la [fix ok] Sección \@ref(caret-cv) presentamos el siguiente código para que se ajuste a un modelo kNN:
