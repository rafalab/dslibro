...
En _machine learning_, nos referimos a esto como  la _Regla de Bayes_. Pero recuerden que esta es una regla teórica ya que en la práctica no sabemos $p_k(\mathbf{x}),k=1,\dots,K$. De hecho, estimar estas probabilidades condicionales puede considerarse como el principal desafío de _machine learning_. Cuanto mejores sean nuestros estimados de la probabilidad $\hat{p}_k(\mathbf{x})$, mejor será nuestro predictor:
