...
Un desafío típico de aprendizaje automático incluirá una gran cantidad de predictores, lo que hace que la visualización sea algo desafiante. Hemos mostrado métodos para visualizar datos univariados y emparejados, pero las gráficas que revelan relaciones entre muchas variables son más complicadas en dimensiones más altas. Por ejemplo, para comparar cada una de las 784 características en nuestro ejemplo de predicción de dígitos, tendríamos que crear, por ejemplo, 306,936 diagramas de dispersión. La creación de un único diagrama de dispersión de los datos es imposible debido a la alta dimensionalidad.

Aquí describimos técnicas poderosas útiles para el análisis exploratorio de datos, entre otras cosas, generalmente conocidas como _dimension reducción_. La idea general es reducir la dimensión del conjunto de datos mientras se conservan características importantes, como la distancia entre características u observaciones. Con menos dimensiones, la visualización se vuelve más factible. La técnica detrás de todo, la descomposición de valores singulares, también es útil en otros contextos. El análisis de componentes principales (PCA) es el enfoque que mostraremos. Antes de aplicar PCA a conjuntos de datos de alta dimensión, motivaremos las ideas detrás con un ejemplo simple.
...
En nuestro ejemplo, para lograr la ortogonalidad, multiplicamos el primer conjunto de coeficientes (primera columna de $A$) por $\sqrt{2}$ y el segundo por $1/\sqrt{2}$, entonces obtenemos la misma distancia exacta si usamos ambas dimensiones:
...
Una forma en que esta información puede ser útil en una aplicación de aprendizaje automático es que podemos reducir la complejidad de un modelo utilizando solo $Z_1$ en lugar de ambos $X_1$ y $X_2$.
...
El ejemplo de dígitos escritos tiene 784 características. ¿Hay espacio para la reducción de datos? ¿Podemos crear algoritmos simples de aprendizaje automático con menos funciones?
...
Ahora transforma el conjunto de prueba:
```{r}
