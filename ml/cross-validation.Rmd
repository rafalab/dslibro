...
Sin embargo, la precisión del set de evaluación es realmente peor que la regresión logística:
...
Entonces, ¿cómo elegimos $k$? En principio, queremos elegir la $k$ que maximiza la exactitud o minimiza el MSE esperado como se define en la Sección \@ref(loss-function). El objetivo de la validación cruzada es estimar estas cantidades para cualquier algoritmo y escoger un set de parámetros de ajuste como $k$. Para entender por qué necesitamos un método especial para hacer esto, repitamos lo que hicimos anteriormente pero para diferentes valores de $k$:
...
Por lo general, intentamos seleccionar una pequeña parte del set de datos para tener la mayor cantidad de datos posible para entrenar. Sin embargo, también queremos que el set de evaluación sea grande para que podamos obtener un estimado estable de la pérdida sin ajustar un número poco práctico de modelos. Las opciones típicas son usar del 10% al 20% de los datos para la evaluación.
...
Para cada set de parámetros de algoritmo que se considera, queremos un estimado del MSE y luego elegiremos los parámetros con el MSE más pequeño. La validación cruzada nos da este estimado.
...
3\.[fix] Cree un índice `ind` con los números de columna de los predictores que eran "estadísticamente significativos" asociados con `y`. Defina "estadísticamente significativo" como un valor-p menor que 0.01.  ¿Cuántos predictores sobreviven este umbral?
...
Tengan en cuenta que un intervalo de confianza construido con _bootstrap_ está mucho más cerca a uno construido con la distribución teórica:
...
[fix check "equal partitions" is that different from equal parts?] _Tengan en cuenta que en la validación cruzada podemos usar ideas similares a esas utilizadas con bootstrap: en lugar de dividir los datos en partes iguales, simplemente usamos muestras bootstrap repetidas veces._
...
[fix check] ¿Cuántas veces aparecen `3`, `4` y `7` en el primer índice re-muestrado?
