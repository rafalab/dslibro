...
En este capítulo presentamos la validación cruzada (_cross validation_ en inglés), una de las ideas más importantes de _machine learning_. Aquí nos centramos en los aspectos conceptuales y matemáticos. Describiremos cómo implementar la validación cruzada en la práctica con el paquete __caret__ en la Sección \@ref(caret-cv) del proximo capitulo. Para motivar el concepto, utilizaremos los dos datos de dígitos predictores presentados en la Sección \@ref(two-or-seven) e introduciremos, por primera vez, un algoritmo real de _machine learning_: _k vecinos más cercanos_ (_k-nearest neighbors_ o kNN por sus siglas en inglés).

## Motivación con k vecinos más cercanos {#knn-cv-intro}

Comencemos cargando los datos y mostrando un gráfico de los predictores con resultados representados con color.
...
como se define en la Sección \@ref(smoothing-ml-connection). Con kNN estimamos $p(x_1, x_2)$ de manera similar a la suavización de compartimientos. Sin embargo, como veremos, kNN es más fácil de adaptar a múltiples dimensiones. Primero definimos la distancia entre todas las observaciones en función de las características. Entonces, para cualquier punto $(x_1,x_2)$ para el cual queremos un estimado de $p(x_1, x_2)$ buscamos los $k$ puntos más cercanos a $(x_1,x_2)$ y luego tomamos un promedio de los 0 y 1 asociados con estos puntos. Nos referimos al conjunto de puntos utilizados para calcular el promedio como el _vecindario_ (_neighborhood_ en inglés). Debido a la conexión que describimos anteriormente entre las expectativas condicionales y las probabilidades condicionales, esto nos da un estimado  $\hat{p}(x_1,x_2)$, al igual que el suavizador de compartimiento nos dio un estimado de una tendencia. Como con los suavizadores de compartimientos, podemos controlar la flexibilidad de nuestro estimado, en este caso a través del parámetro $k$: $k$s más grandes resultan en estimados más suaves, mientras que $k$s más pequeñas resultan en estimados más flexibles y más ondulados.


Para implementar el algoritmo, podemos usar la función `knn3` del paquete __caret__. Mirando el archivo de ayuda para este paquete, vemos que podemos llamarlo de una de dos maneras. Utilizaremos el primero en el que especificamos una _formula_ y un _data frame_. El _data frame_ contiene todos los datos que se utilizarán. La fórmula tiene  la forma `outcome ~ predictor_1 + predictor_2 + predictor_3` y así sucesivamente. Por lo tanto, escribiríamos `y ~ x_1 + x_2`. Si vamos a usar todos los predictores, podemos usar el `.` así `y ~ .`. La última llamada se ve así:
...
La función `predict` para `knn` produce una probabilidad para cada clase. Guardamos la probabilidad de ser un 7 como el estimado $\hat{p}(x_1, x_2)$:
...
En la Sección \@ref(two-or-seven) utilizamos la regresión lineal para generar un estimado.
```{r}
...
Vemos que kNN se adapta mejor a la forma no lineal de $p(x_1, x_2)$. Sin embargo, nuestro estimado tiene algunas islas de azul en el área roja, lo que intuitivamente no tiene mucho sentido. Esto se debe a lo que  llamamos _sobreentrenamiento_ (_overtraining_ en inglés). Describimos el _sobreentrenamiento_ en detalle a continuación. Este es la razón por la que tenemos una mayor exactitud en el set de entrenamiento en comparación con el set de evaluación:
...
El _sobreentrenamiento_ es peor cuando fijamos $k=1$. Con $k=1$, el estimado para cada $(x_1, x_2)$ en el set de entrenamiento se obtiene solo con la $y$ correspondiente a ese punto. En este caso, si el $(x_1, x_2)$ son únicos, obtendremos una exactitud perfecta en el set de entrenamiento porque cada punto se usa para predecirse a sí mismo. Recuerden que si los predictores no son únicos y tienen resultados diferentes para al menos un set de predictores, entonces es imposible predecir perfectamente.
...
Sin embargo, la precisión del set de evaluación es en realidad peor que la regresión logística:
...
El estimado $\hat{p}(x_1, x_2)$ sigue los datos de entrenamiento muy de cerca (izquierda). Pueden ver que en el set de entrenamiento, los límites se han trazado para rodear perfectamente un punto rojo único en un mar azul. Porque la mayoría de los puntos $(x_1, x_2)$ son únicos, la predicción es 1 o 0 y la predicción para ese punto es la etiqueta asociada. Sin embargo, tan pronto presentamos el set de entrenamiento (derecha), vemos que muchas de estas pequeñas islas ahora tienen el color opuesto y terminamos haciendo varias predicciones incorrectas.
...
Entonces, ¿cómo elegimos $k$? En principio queremos elegir la $k$ que maximiza la exactitud o minimiza el MSE esperado como se define en la Sección \@ref(loss-function). El objetivo de la validación cruzada es estimar estas cantidades para cualquier algoritmo y escoger un conjunto de parámetros de ajuste como $k$. Para entender por qué necesitamos un método especial para hacer esto, repitamos lo que hicimos anteriormente pero para diferentes valores de $k$:
...
Tengan en cuenta que estimamos la exactitud utilizando tanto el set de entrenamiento como el set de evaluación. Ahora podemos graficar las estimados de exactitud para cada valor de $k$:
...
2. Si entrenamos un algoritmo en el mismo set de datos que usamos para calcular el error aparente, podríamos estar entrenando demasiado. En general, cuando hacemos esto, el error aparente será una subestimación del error verdadero. Veremos un ejemplo extremo de esto con _k vecinos más cercanos_.
...
El primero que describimos es _validación cruzada K-fold_ (_K-fold cross validation_ en inglés). En términos generales, un desafío de _machine learning_ comienza con un set de datos (azul en la imagen a continuación). Necesitamos construir un algoritmo usando este set de datos que eventualmente se usará en sets de datos completamente independientes (amarillo).
...
Entonces, para imitar esta situación, creamos una parte de nuestro set de datos y nos imaginamos que es un set de datos independiente: dividimos el set de datos en un _set de entrenamiento_ (azul) y un _set de evaluación_ (rojo). Entrenaremos nuestro algoritmo exclusivamente en el set de entrenamiento y usaremos el set de evaluación solo para fines de evaluación.

Por lo general, intentamos seleccionar una pequeña parte del set de datos para tener la mayor cantidad de datos posible para entrenar. Sin embargo, también queremos que el set de evaluación sea grande para que podamos obtener un estimado estable de la pérdida sin ajustar un número poco práctico de modelos. Las opciones típicas son usar del 10% al 20% de los datos para las pruebas.

```{r, echo=FALSE}
knitr::include_graphics("ml/img/cv-3.png")
```

Reiteremos que es indispensable que no usemos el set de evaluación para nada: no para filtrar filas, no para seleccionar características, ¡nada!
...
Para cada conjunto de parámetros de algoritmo que se considera, queremos un estimado del MSE y luego elegiremos los parámetros con el MSE más pequeño. La validación cruzada nos da este estimado.

Primero, antes de comenzar el procedimiento de validación cruzada, es importante corregir todos los parámetros del algoritmo. Aunque entrenaremos el algoritmo en el conjunto de sets de entrenamiento, los parámetros $\lambda$ serán los mismos en todos los sets de entrenamiento. Usaremos $\hat{y}_i(\lambda)$ para denotar los predictores obtenidos cuando usamos parámetros $\lambda$.
...
queremos considerar sets de datos que puedan considerarse una muestra aleatoria independiente y queremos hacerlo varias veces. Con la validación cruzada _K-fold_, lo hacemos $K$ veces. En los dibujos animados, estamos mostrando un ejemplo que usa $K=5$.
...
Tengan en cuenta que esta es solo una muestra y, por lo tanto, devolverá un estimado ruidoso del error verdadero. Por eso tomamos $K$ muestras, no solo una. En la validación _K-cross_, dividimos aleatoriamente las observaciones en $K$ sets no superpuestos:
...
Ahora repetimos el cálculo anterior para cada uno de estos sets $b=1,\dots,K$ y obtenemos $\hat{\mbox{MSE}}_1(\lambda),\dots, \hat{\mbox{MSE}}_K(\lambda)$. Luego, para nuestro estimado final, calculamos el promedio:

$$
\hat{\mbox{MSE}}(\lambda) = \frac{1}{B} \sum_{b=1}^K \hat{\mbox{MSE}}_b(\lambda)
$$

y obtenemos un estimado de nuestra pérdida. Un paso final sería seleccionar el $\lambda$ que minimiza el MSE.

Hemos descrito cómo usar la validación cruzada para optimizar los parámetros. Sin embargo, ahora tenemos que tener en cuenta el hecho de que la optimización se produjo en los datos de entrenamiento y, por lo tanto, necesitamos un estimado de nuestro algoritmo final basado en datos que no se utilizaron para optimizar la elección. Aquí es donde usamos el set de evaluación que separamos al principio:
...
3\. Cree un índice `ind` con los números de columna de los predictores que eran "estadísticamente significativos" asociados con `y`. Utilice un valor de corte p de 0.01 para definir "estadísticamente significativo".  ¿Cuántos predictores sobreviven  este umbral?
...
5\. Vuelva a ejecutar la validación cruzada nuevamente, pero esta vez usando kNN. Pruebe la siguiente cuadrícula de parámetros de ajuste: `k = seq(101, 301, 25)`. Haga un gráfico de la exactitud resultante.

6\. En los ejercicios 3 y 4, vemos que a pesar del hecho de que `x` y `y` son completamente independientes, pudimos predecir `y` con una exactitud superior al 70%. Debemos estar haciendo algo mal entonces. ¿Qué es?

a. La función `train` estima la exactitud en los mismos datos que usa para entrenar el algoritmo.
...
Supongamos que la distribución del ingreso de su población es la siguiente:
...
Si conocemos esta distribución, podemos construir un intervalo de confianza. El problema aquí es que, como ya hemos descrito, en la práctica no sabemos la distribución. En el pasado, hemos usado el teorema del límite central (CLT), pero el CLT que estudiamos se aplica a los promedios y aquí estamos interesados en la mediana. Podemos ver que el intervalo de confianza del 95% basado en CLT:
...
*Tengan en cuenta que en la validación cruzada podemos usar ideas similares a esas utilizadas con _bootstrap_ : en lugar de dividir los datos en particiones iguales, simplemente usamos muestras _bootstrap_ repetidas veces.
...
¿Cuántas veces `3`, `4` y `7` aparecen en el primer índice en las 10 muestras en `indexes`?


2\. Vemos que algunos números aparecen más de una vez y otros no aparecen ninguna vez. Esto tiene que ser así para que cada set de datos sea independiente. Repita el ejercicio para todos los índices muestreados.
...
Estime el cuantil 75, que sabemos es:
