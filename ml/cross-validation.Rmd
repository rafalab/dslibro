...
En este capítulo presentamos la validación cruzada (_cross validation_ en inglés), una de las ideas más importantes de _machine learning_. Aquí nos centramos en los aspectos conceptuales y matemáticos. Describiremos cómo implementar la validación cruzada en la práctica con el paquete __caret__ en la Sección \@ref(caret-cv) del proximo capitulo. Para motivar el concepto, utilizaremos los dos datos de dígitos predictores presentados en la Sección \@ref(two-or-seven) e introduciremos, por primera vez, un algoritmo real de _machine learning_: _k-nearest neighbors_ (kNN).
...
$$
como se define en la Sección \@ref(smoothing-ml-connection). Con _k-nearest neighbors_ (kNN) estimamos $p(x_1, x_2)$ de manera similar a la suavización de compartimientos. Sin embargo, como veremos, kNN es más fácil de adaptar a múltiples dimensiones. Primero definimos la distancia entre todas las observaciones en función de las características. Entonces, por cualquier punto $(x_1,x_2)$ para lo cual queremos un estimado de $p(x_1, x_2)$ buscamos los $k$ puntos más cercanos a $(x_1,x_2)$ y luego tomamos un promedio de los 0 y 1 asociados con estos puntos. Nos referimos al conjunto de puntos utilizados para calcular el promedio como el _neighborhood_ [fix translate?]. [fix read next 2 sent; check ok] Debido a la conexión que describimos anteriormente entre las expectativas condicionales y las probabilidades condicionales, esto nos da un $\hat{p}(x_1,x_2)$, al igual que el suavizador de compartimiento nos dio un estimado de una tendencia. Como con los suavizadores de compartimientos, podemos controlar la flexibilidad de nuestro estimado, en este caso a través del parámetro $k$: $k$ más grandes resultan en estimados más suaves, mientras que  $k$ más pequeñas resultan en estimados más flexibles y más ondulados.


Para implementar el algoritmo, podemos usar la función `knn3` del paquete __caret__. Mirando el archivo de ayuda para este paquete, vemos que podemos llamarlo de una de dos maneras. Utilizaremos el primero en el que especificamos una _formula_ y un _data frame_. El _data frame_ contiene todos los datos que se utilizarán. La fórmula tiene [fix "and so on"] la forma `outcome ~ predictor_1 + predictor_2 + predictor_3` etcétera. Por lo tanto, escribiríamos `y ~ x_1 + x_2`. Si vamos a usar todos los predictores, podemos usar el `.` así `y ~ .`. La última llamada se ve así:
...
Para esta función, también debemos elegir un parámetro: el número [fix vecinos?] de _neighbors_ para incluir. Comencemos con el valor predeterminado $k=5$.
...
La función `predict` para `knn` produce una probabilidad para cada clase. Mantenemos la probabilidad de ser un 7 como el estimado $\hat{p}(x_1, x_2)$
...
Vemos que kNN se adapta mejor a la forma no lineal de $p(x_1, x_2)$. Sin embargo, nuestro estimado tiene algunas islas de azul en el área roja, lo que intuitivamente no tiene mucho sentido. Esto se debe a lo que [fix] llamamos _over-training_. Describimos el _over-training_ en detalle a continuación. Tal es la razón por la que tenemos una mayor exactitud en el set de entrenamiento en comparación con el set de evaluación:
...
El _over-training_ es peor cuando fijamos $k=1$. Con $k=1$, el estimado para cada $(x_1, x_2)$ en el set de entrenamiento se obtiene solo con el $y$ correspondiente a ese punto. En este caso, si el $(x_1, x_2)$ son únicos, obtendremos una exactitud perfecta en el set de entrenamiento porque cada punto se usa para predecirse a sí mismo. Recuerde que si los predictores no son únicos y tienen resultados diferentes para al menos un set de predictores, entonces es imposible predecir perfectamente.
...
El estimado $\hat{p}(x_1, x_2)$ sigue los datos de entrenamiento muy de cerca (izquierda). Puedes ver que en el set de entrenamiento, los límites se han trazado para rodear perfectamente un único punto rojo en un mar azul. Porque la mayoría de los puntos $(x_1, x_2)$ son únicos, la predicción es 1 o 0 y la predicción para ese punto es la etiqueta asociada. Sin embargo, una vez que presentamos el set de entrenamiento (derecha), vemos que muchas de estas pequeñas islas ahora tienen el color opuesto y terminamos haciendo varias predicciones incorrectas.
...
Primero, noten que el estimado obtenido en el set de entrenamiento es generalmente menor que el estimado obtenido con el set de evaluación, con una diferencia mayor para valores más pequeños de $k$. Esto se debe al [fix]  sobreentrenamiento. También tengan en cuenta que la exactitud frente a $k$ el gráfico es bastante irregular. No esperamos esto porque pequeños cambios en $k$ no deberían afectar demasiado [fix rendimiento ok?] el rendimiento del algoritmo. La irregularidad se explica por el hecho de que la exactitud se calcula en una muestra y, por lo tanto, es una variable aleatoria. Esto demuestra por qué preferimos minimizar la pérdida esperada en lugar de la pérdida que observamos con un set de datos.
...
Otra razón por la que necesitamos un mejor estimado de exactitud es que si usamos el set de evaluación para elegir esta $k$, [fix] no debemos esperar que el estimado de exactitud que acompaña se extrapole al mundo real. Esto se debe a que incluso aquí rompimos una regla de oro dl _machine learning_: seleccionamos $k$ utilizando el set de evaluación. La validación cruzada también ofrece un estimado que toma esto en cuenta.
...
El primero que describimos es _validación cruzada K-fold_ (_K-fold cross validation_ en inglés).
...
Reiteremos que es indispensable que no usemos el set de evaluación en absoluto: no para filtrar filas, no para seleccionar características, ¡nada!
...
Para cada conjunto de parámetros de algoritmo que se considera, queremos un estimado del MSE y luego elegiremos los parámetros con el MSE más pequeño. La validación cruzada proporciona este estimado.
...
queremos considerar sets de datos que puedan considerarse una muestra aleatoria independiente y queremos hacerlo varias veces. Con la validación cruzada K-fold, lo hacemos $K$ veces. En los dibujos animados, estamos mostrando un ejemplo que usa $K=5$.
...
Ahora podemos ajustar el modelo en el set de entrenamiento, luego calcular el error aparente en el conjunto independiente:

$$
\hat{\mbox{MSE}}_b(\lambda) = \frac{1}{M}\sum_{i=1}^M \left(\hat{y}_i^b(\lambda) - y_i^b\right)^2
$$

Tengan en cuenta que esta es solo una muestra y, por lo tanto, devolverá un estimado ruidosa del error verdadero. Por eso tomamos $K$ muestras, no solo una. En la validación de K-cross, dividimos aleatoriamente las observaciones en $K$ conjuntos no superpuestos:
...
Ahora repetimos el cálculo anterior para cada uno de estos conjuntos $b=1,\dots,K$ y obtener $\hat{\mbox{MSE}}_1(\lambda),\dots, \hat{\mbox{MSE}}_K(\lambda)$. Luego, para nuestro estimado final, calculamos el promedio:
...
Una forma de mejorar la varianza de nuestro estimado final es tomar más muestras. Para hacer esto, ya no necesitaríamos que el set de entrenamiento se dividiera en conjuntos no superpuestos. En cambio, simplemente elegiríamos $K$ conjuntos de algún tamaño al azar.
...
*Tengan en cuenta que en la validación cruzada podemos usar ideas similares a las utilizadas en el _bootstrap_ : en lugar de dividir los datos en particiones iguales, simplemente usamos muchas muestras _bootstrap_.*
