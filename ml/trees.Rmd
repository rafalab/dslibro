...
En general, incluir el 10% de los datos en un caso con $p$ dimensiones, necesitamos un intervalo con cada lado del tamaño $\sqrt[p]{.10}$ del total. Esta proporción se acerca a 1 rápidamente, y si la proporción es 1 significa que incluimos todos los datos y ya no estamos suavizando.
...
Cuando llegamos a 100 predictores, el vecindario ya no es muy local, ya que cada lado cubre casi todo el conjunto de datos.
...
Para motivar esta sección, utilizaremos un nuevo conjunto de datos.
eso incluye el desglose de la composición del aceite de oliva en 8 ácidos grasos:
...
Quitamos el `area` columna porque no lo usaremos como predictor.
...
¡Esto implica que deberíamos ser capaces de construir un algoritmo que prediga perfectamente! Podemos ver esto claramente al trazar los valores para eicosenoico y linoleico.
...
En la sección \@ref(predictor-space) definimos espacios predictores. El espacio predictivo aquí consiste en puntos de ocho dimensiones con valores entre 0 y 100. En la gráfica anterior, mostramos el espacio definido por los dos predictores eicosenoico y linoleico y, a simple vista,
podemos construir una regla de predicción que particione el espacio del predictor para que cada partición contenga solo resultados de una categoría. Esto a su vez se puede utilizar para definir un algoritmo con una precisión perfecta. Específicamente, definimos la siguiente regla de decisión. Si el eicosenoico es mayor que 0.065, prediga el sur de Italia. Si no, entonces si linoleico es más grande que $10.535$, predicen Cerdeña, y si es más bajo, predicen el norte de Italia. Podemos dibujar este árbol de decisión así:
...
Un árbol es básicamente un diagrama de flujo de preguntas de sí o no. La idea general de los métodos que estamos describiendo es definir un algoritmo que use datos para crear estos árboles con predicciones en los extremos, conocidos como _nodes_.
Los árboles de regresión y decisión operan prediciendo una variable de resultado $Y$ dividiendo los predictores.


### Árboles de regresión

Cuando el resultado es continuo, llamamos al método un árbol _regresión_. Para introducir árboles de regresión, utilizaremos los datos de la encuesta de 2008 utilizados en secciones anteriores para describir la idea básica de cómo construimos estos algoritmos. Al igual que con otros algoritmos de aprendizaje automático, intentaremos estimar la expectativa condicional $f(x) = \mbox{E}(Y | X = x)$ con $Y$ el margen de la encuesta y $x$ el dia.
...
Una vez que seleccionamos una partición $\mathbf{x}$ para dividir para crear las nuevas particiones, encontramos un predictor $j$ y valor $s$ que definen dos nuevas particiones, que llamaremos $R_1(j,s)$ y $R_2(j,s)$, que divide nuestras observaciones en la partición actual preguntando si $x_j$ es mayor que $s$:
...
Pero como elegimos $j$ y $s$? Básicamente encontramos el par que minimiza la suma residual del cuadrado (RSS):
$$
...
Esto se aplica recursivamente a las nuevas regiones. $R_1$ y $R_2$. Describimos cómo nos detenemos más tarde, pero una vez que terminamos de dividir el espacio del predictor en regiones, en cada región se realiza una predicción utilizando las observaciones en esa región.

Echemos un vistazo a lo que hace este algoritmo en los datos de la encuesta de elecciones presidenciales de 2008. Utilizaremos el `rpart` funcionar en el paquete __rpart__.
...
Aquí, solo hay un predictor. Por lo tanto, no tenemos que decidir qué predictor $j$ para dividir, simplemente tenemos que decidir qué valor $s$ solíamos dividirnos. Podemos ver visualmente dónde se hicieron las divisiones:
...
La primera división se realiza el día 39.5. Una de esas regiones se divide en el día 86.5. Las dos nuevas particiones resultantes se dividen en los días 49.5 y 117.5, respectivamente, y así sucesivamente. Terminamos con 8 particiones. La estimación final $\hat{f}(x)$ se ve como esto:
...
Tenga en cuenta que el algoritmo dejó de particionar a las 8. Ahora explicamos cómo se toma esta decisión.

Primero necesitamos definir el término _parámetro de complejidad_ (cp). Cada vez que dividimos y definimos dos nuevas particiones, nuestro conjunto de entrenamiento RSS disminuye. Esto se debe a que con más particiones, nuestro modelo tiene más flexibilidad para adaptarse a los datos de entrenamiento. De hecho, si divide hasta que cada punto sea su propia partición, entonces RSS baja hasta 0 ya que el promedio de un valor es el mismo valor. Para evitar esto, el algoritmo establece un mínimo de cuánto debe mejorar el RSS para que se agregue otra partición. Este parámetro se conoce como _parámetro de complejidad_ (cp). El RSS debe mejorar por un factor de cp para que se agregue la nueva partición. Por lo tanto, los valores grandes de cp obligarán al algoritmo a detenerse antes, lo que resulta en menos nodos.

Sin embargo, cp no es el único parámetro utilizado para decidir si debemos particionar una partición actual o no. Otro parámetro común es el número mínimo de observaciones requeridas en una partición antes de dividirla más. El argumento usado en el `rpart` la función es `minsplit` y el valor predeterminado es 20. El `rpart` la implementación de árboles de regresión también permite a los usuarios determinar un número mínimo de observaciones en cada nodo. El argumento es `minbucket` y por defecto a `round(minsplit/3)`.
...
La primera diferencia es que formamos predicciones calculando qué clase es la más común entre las observaciones del conjunto de entrenamiento dentro de la partición, en lugar de tomar el promedio en cada partición (ya que no podemos tomar el promedio de las categorías).
...
1\. Construir $B$ árboles de decisión utilizando el conjunto de entrenamiento. Nos referimos a los modelos equipados como $T_1, T_2, \dots, T_B$. Luego explicamos cómo nos aseguramos de que sean diferentes.

2\. Para cada observación en el conjunto de prueba, forme una predicción $\hat{y}_j$ usando el árbol $T_j$.
...
Entonces, ¿cómo obtenemos diferentes árboles de decisión de un solo conjunto de entrenamiento? Para esto, usamos la aleatoriedad de dos maneras que explicamos en los pasos a continuación. Dejar $N$ sea el número de observaciones en el conjunto de entrenamiento. Crear $T_j, \, j=1,\ldots,B$ del conjunto de entrenamiento hacemos lo siguiente:

1\. Crear un conjunto de entrenamiento de bootstrap por muestreo $N$ observaciones del conjunto de entrenamiento **con reemplazo**. Esta es la primera forma de inducir aleatoriedad.

2\. Una gran cantidad de características es típica en los desafíos de aprendizaje automático. A menudo, muchas características pueden ser informativas, pero incluirlas todas en el modelo puede resultar en un sobreajuste. La segunda forma en que los bosques aleatorios inducen la aleatoriedad es seleccionando aleatoriamente las características que se incluirán en la construcción de cada árbol. Se selecciona un subconjunto aleatorio diferente para cada árbol. Esto reduce la correlación entre los árboles en el bosque, mejorando así la precisión de la predicción.
...
Tenga en cuenta que si aplicamos la función `plot` al objeto resultante, almacenado en `fit`, vemos cómo cambia la tasa de error de nuestro algoritmo a medida que agregamos árboles.
...
Para definir _importancia variable_ contamos con qué frecuencia se usa un predictor en los árboles individuales. Puede obtener más información sobre _variable importancia_ en un libro de aprendizaje automático avanzado^[https://web.stanford.edu/~hastie/Papers/ESLII.pdf]. El paquete __caret__ incluye la función `varImp` que extrae importancia variable de cualquier modelo en el que se implementa el cálculo. Damos un ejemplo de cómo usamos la importancia variable en la siguiente sección.


## Ejercicios

1\. Cree un conjunto de datos simple donde el resultado crezca 0.75 unidades en promedio por cada aumento en un predictor:
...
9\. Utilizar el `rpart` función para ajustar un árbol de clasificación a la `tissue_gene_expression` conjunto de datos Utilizar el `train` función para estimar la precisión. Poner a prueba o probar `cp` valores de `seq(0, 0.05, 0.01)`. Trace la precisión para informar los resultados del mejor modelo.
