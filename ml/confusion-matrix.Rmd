...
Una forma estándar de generar los sets de entrenamiento y evaluación es dividiendo aleatoriamente los datos. El paquete __caret__ incluye la función `createDataPartition` que nos ayuda a generar índices para dividir aleatoriamente los datos en sets de entrenamiento y evaluación:
...
¿Podemos mejorarla? El análisis de datos exploratorios sugiere que podemos porque, en promedio, los hombres son un poco más altos que las mujeres:
...
Ahora podemos probar este umbral en nuestro set de evaluaciones para asegurarnos de que nuestra exactitud no sea demasiado optimista:
...
La función `F_meas` en el paquete __caret__ calcula este resumen [fix] con `beta` por defecto a 1.
...
[fix check] Vemos que se maximiza en una medida $F_1$ de:
...
Ahora vemos que hacemos mucho mejor que adivinar, que tanto la sensibilidad como la especificidad son relativamente altas, y que hemos construido nuestro primer algoritmo de _machine learning_. Este toma altura como predictor y predice mujeres si la persona mide 65 pulgadas o menos.


### La prevalencia importa en la práctica

[fix] Un algoritmo de _machine learning_ con sensibilidad y especificidad muy altas puede ser inútil en la práctica cuando la prevalencia es cercana a 0 o 1. Para ver esto, considere el caso de una doctora que se especializa en una enfermedad rara y que está interesada en desarrollar un algoritmo para predecir quién tiene la enfermedad. La doctora comparte datos con ustedes, que entonces desarrollan un algoritmo con una sensibilidad muy alta. Explican que esto significa que si un paciente tiene la enfermedad, es muy probable que el algoritmo prediga correctamente. También le dicen a la doctora que están preocupados porque, según el set de datos que analizaron, la mitad de los pacientes tienen la enfermedad: $\mbox{Pr}(\hat{Y}=1)$. La doctora no está preocupada ni impresionada y explica que lo importante es la precisión de la evaluación: $\mbox{Pr}(Y=1 | \hat{Y}=1)$. Usando el teorema de Bayes, podemos conectar las dos medidas:
...
Al comparar los dos métodos (adivinar versus usar un umbral de altura), observamos la exactitud y [fix el?] $F_1$. El segundo método claramente superó al primero. Sin embargo, si bien consideramos varios umbrales para el segundo método, para el primero solo consideramos un enfoque: adivinar con igual probabilidad. Tengan en cuenta que adivinar `Male` con mayor probabilidad nos daría una mayor exactitud debido al sesgo en la muestra:
...
Recuerden que para cada uno de estos parámetros, [fix] podemos obtener una sensibilidad y especificidad diferente. Por esta razón, un enfoque muy común para evaluar métodos es compararlos gráficamente trazando ambos.

Un gráfico ampliamente utilizado que hace esto es la curva ROC [fix discuss with rafa] (_acrónimo de Receiver Operating Characteristic_ o Característica Operativa del Receptor). Si se pregunta de dónde viene este nombre, puede consultar la página de Wikipedia Curva ROC^[https://es.wikipedia.org/wiki/Curva_ROC].
...
Podemos ver que obtenemos una mayor sensibilidad con este enfoque para todos los valores de especificidad, lo que implica que, de hecho, es un método mejor. Tengan en cuenta que las curvas ROC para adivinar siempre caen en la línea de identificación. También noten que al hacer curvas ROC, a menudo es bueno agregar el umbral asociado con cada punto.

Los paquetes __pROC__y__plotROC__ son útiles para generar estos gráficos.

Las curvas ROC tienen una debilidad y es que ninguna de las medidas graficadas depende de la prevalencia. En los casos en que la prevalencia es importante, en su lugar podemos hacer un gráfico de precisión-_recall_. La idea es similar, pero en cambio graficamos la precisión versus el _recall_:
...
De este gráfico vemos de inmediato que la precisión de adivinar no es alta. Esto se debe a que la prevalencia es baja. También vemos que si cambiamos los positivos para que signifiquen Masculino en lugar de Femenino, la curva ROC permanece igual, pero el gráfico de precisión-_recall_ cambia.
...
Debido a que a menudo tenemos un set de evaluaciones con muchas observaciones, digamos $N$, usamos el error cuadrático medio (MSE por sus siglas en inglés):

$$
\mbox{MSE} = \frac{1}{N} \mbox{RSS} = \frac{1}{N}\sum_{i=1}^N (\hat{y}_i - y_i)^2
$$

En la práctica, a menudo informamos [fix] el _root_ error cuadrático medio/	desviación cuadrática media (RMSE), que es $\sqrt{\mbox{MSE}}$, porque está en las mismas unidades que los resultados. Pero hacer las matemáticas a menudo es más fácil con el MSE y, por lo tanto, se usa más en los libros de texto, ya que estos generalmente describen las propiedades teóricas de los algoritmos.

[fix] Si los resultados son binarios, tanto RMSE como MSE son equivalentes a una exactitud menos uno, ya que $(\hat{y} - y)^2$ es 0 si la predicción fue correcta y 1 en caso contrario. En general, nuestro objetivo es construir un algoritmo que minimice la pérdida para que esté lo más cerca posible de 0.
...
Este es un concepto teórico porque en la práctica solo tenemos un set de datos para trabajar. Pero en teoría, pensamos en tener una gran cantidad de muestras aleatorias (llámela $B$), aplique nuestro algoritmo a cada una, obtenga un MSE para cada muestra aleatoria y piense en el MSE esperado como:
...
Tengan en cuenta que hay funciones de pérdida distintas de la pérdida al cuadrado. Por ejemplo, [fix desviación promedia ; desviación media ; error medio absoluto] el _Mean Absolute Error_ utiliza valores absolutos, $|\hat{Y}_i - Y_i|$ en lugar de cuadrar los errores
...
5\. Ahora use las funciiones `sensitivity` y `specificity` para informar especificidad y sensibilidad.
