...
Antes de continuar aprendiendo sobre algoritmos de _machine learning_, presentamos el importante concepto de _suavización_ (_smoothing_ en inglés). La suavización es una técnica muy poderosa utilizada en todo el análisis de datos. Otros nombres dados a esta técnica [fix] son _ajustamiento de curvas_ y _filtrado de paso bajo_ (_curve fitting_ y _low pass filtering_ en inglés). [fix] La suavización está diseñada para detectar tendencias en presencia de datos ruidosos en casos en los que se desconoce la forma de la tendencia. El nombre _suavización_ proviene del hecho de que para lograr esta hazaña, asumimos que la tendencia es _suave_, como en una superficie lisa. En contraste, el ruido, o la desviación de la tendencia, es impredecible tambaleante:
...
**Para entender por qué cubrimos este tema, recuerden que los conceptos detrás de las técnicas de suavización son extremadamente útiles en _machine learning_ porque las expectativas/probabilidades condicionales pueden considerarse como tendencias de formas desconocidas que necesitamos estimar en presencia de incertidumbre.**

Para explicar estos conceptos, nos centraremos primero en un problema con un solo predictor. Específicamente, tratamos de estimar la tendencia temporal en el margen de la encuesta de votación popular de 2008 en Estados Unidos (la diferencia entre Obama y McCain).
...
Para los propósitos de este ejemplo, no lo piensem como un problema de pronóstico. En cambio, simplemente estamos interesados en aprender la forma de la tendencia *después* de que terminen las elecciones.
...
Para pensar en esto como un problema de _machine learning_, tengan en cuenta que queremos predecir $Y$ dado un día $x$. Si supiéramos la expectativa condicional $f(x) = \mbox{E}(Y \mid X=x)$, lo usaríamos Pero como no conocemos esta expectativa condicional, tenemos que estimarla. Usemos la regresión, ya que es el único método que hemos aprendido hasta ahora.
...
La idea general del suavización es agrupar los puntos de datos en estratos en los que el valor de $f(x)$ se puede suponer que es constante. Podemos hacer esta suposición porque pensamos $f(x)$ cambia lentamente y, como resultado, $f(x)$ es casi constante en pequeñas ventanas de tiempo. Un ejemplo de esta idea para los datos `poll_2008` es suponer que la opinión pública se mantuvo aproximadamente igual en el plazo de una semana. Con este supuesto, tenemos varios puntos de datos con el mismo valor esperado.

Si fijamos un día para estar en el centro de nuestra semana, llámelo $x_0$, entonces para cualquier otro día $x$ tal que $|x - x_0| \leq 3.5$, asumimos que $f(x)$ es una constante $f(x) = \mu$. Esta suposición implica que:
$$
E[Y_i | X_i = x_i ] \approx \mu \mbox{ if } |x_i - x_0| \leq 3.5
$$

Al suavizar, llamamos al tamaño del intervalo que satisface $|x_i - x_0| \leq 3.5$, el _tamaño de la ventana_, _parámetro de suavizado_ o _span_. Más adelante veremos que intentamos optimizar este parámetro.

Esta suposición implica que una buen estimado de $f(x)$ es el promedio de $Y_i$ valores en la ventana. Si definimos $A_0$ como el conjunto de índices $i$ tal que $|x_i - x_0| \leq 3.5$ y $N_0$ como el número de índices en $A_0$, entonces nuestro estimado es:
...
Al calcular esta media para cada punto, formamos un estimado de la curva subyacente $f(x)$. A continuación, mostramos el procedimiento que se produce a medida que avanzamos de -155 a 0. En cada valor de $x_0$, mantenemos el estimado $\hat{f}(x_0)$ y continuamos al siguiente punto:
...
El código final y el estimado resultante se ven así:
...
El resultado final de suavización de compartimiento es bastante ondulante. Una razón para esto es que cada vez que la ventana se mueve, cambian dos puntos. Podemos atenuar esto algo tomando promedios ponderados que le dan al punto central más peso que a los puntos lejanos, con los dos puntos en los bordes recibiendo muy poco peso.
...
en el que cada punto recibe un peso de $0$ o $1/N_0$, con $N_0$ el número de puntos en la semana. En el código anterior, usamos el argumento `kernel="box"` en nuestra llamada a la función `ksmooth`. Esto se debe a que [fix] la función de peso parece una caja. La función `ksmooth` ofrece una opción "más suave" que utiliza la densidad normal para asignar pesos.
...
Tengan en cuenta que el estimado final ahora se ve más suave.

Hay varias funciones en R que implementan suavizadores de compartimientos. Un ejemplo es `ksmooth`, mostrada anteriormente. En la práctica, sin embargo, generalmente preferimos métodos que usan modelos ligeramente más complejos que ajustar una constante. El resultado final anterior, por ejemplo, todavía es algo ondulante en partes que no esperamos que sea (entre -125 y -75, por ejemplo). Métodos como `loess`, que explicamos a continuación, mejoran esto.

## Regresión ponderada local (loess)

[fix check sent] Una limitación del enfoque de suavización de compartimientos que acabamos de describir es que necesitamos ventanas pequeñas para mantener los supuestos aproximadamente constantes. Como resultado, terminamos con un pequeño número de puntos de datos [fix 'to average'] para promediar y obtener estimaciones imprecisas $\hat{f}(x)$. Aquí describimos cómo _la regresión ponderada local_ (loess o _local weighted regression_ en inglés) nos permite considerar tamaños de ventana más grandes. [fix check] Para hacer esto, usaremos un resultado matemático, conocido como el teorema de Taylor, que nos dice que si mira con atención a cualquier función suave $f(x)$, se verá como una línea. Para ver por qué esto tiene sentido, consideren los bordes curvos que hacen los jardineros con espadas de bordes rectos:

```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path, "garden.png"))
```

(&Quot;Downing Street garden path edge"^[https://www.flickr.com/photos/49707497@N06/7361631644] por el usuario de Flckr Número 10^[https://www.flickr.com/photos/number10gov/] Licencia CC-BY 2.0^[https://creativecommons.org/licenses/by/2.0/].)

En lugar de suponer que la función es aproximadamente constante en una ventana, suponemos que la función es localmente lineal. [fix check] Podemos considerar tamaños de ventana más grandes con el supuesto lineal que con una constante. En lugar de la ventana de una semana, consideramos una ventana más grande en la que la tendencia es aproximadamente lineal. Comenzamos con una ventana de tres semanas y luego consideramos y evaluamos otras opciones:
...
El valor ajustado en $x_0$ se convierte en nuestro estimado $\hat{f}(x_0)$. A continuación, mostramos el procedimiento que ocurre mientras pasamos de -155 a 0.
...
[fix check] El resultado final es un ajuste más suave que ese producido por suavización de compartimiento porque utilizamos tamaños de muestra más grandes para estimar nuestros parámetros locales:
...
[fix] Diferentes _spans_ nos dan diferentes estimados. Podemos ver cómo diferentes tamaños de ventana conducen a diferentes estimados:
...
Aquí están los estimados finales:
...
### Cómo ajustar parábolas

[fix check title] El teorema de Taylor también nos dice que si miramos cualquier función matemática lo suficientemente cerca, parece una parábola. El teorema también establece que no tienen que mirar tan de cerca cuando se aproxima con parábolas como cuando se aproxima con líneas. Esto significa que podemos hacer que nuestras ventanas sean aún más grandes y ajustar parábolas en lugar de líneas.
...
`ggplot` utiliza loess en su función `geom_smooth`:
...
Pero tengan cuidado con los parámetros predeterminados, ya que rara vez son óptimos.
Sin embargo, pueden cambiarlos convenientemente:
...
Para ver cómo la suavización se relaciona con el _machine learning_ con un ejemplo concreto, consideren nuestro ejemplo \@ref(two-or-seven). [fix eng sound off too ] Si definimos el resultado $Y = 1$ para dígitos que son siete y $Y=0$ para dígitos que son 2, entonces estamos interesados en estimar la probabilidad condicional:

$$
p(x_1, x_2) = \mbox{Pr}(Y=1 \mid X_1=x_1 , X_2 = x_2).
$$
con $X_1$ y $X_2$ los dos predictores definidos en la Sección \@ref(two-or-seven). En este ejemplo, los 0 y 1 que observamos son "ruidosos" porque para algunas regiones las probabilidades $p(x_1, x_2)$ no están tan cerca de 0 o 1. Así que necesitamos estimar $p(x_1, x_2)$. La suavización es una alternativa para lograr esto. En la sección \@ref(two-or-seven) vimos que la regresión lineal no era lo suficientemente flexible como para capturar la naturaleza no lineal de $p(x_1, x_2)$, por lo tanto, los enfoques de suavización pueden proveer una mejora. En el siguiente capítulo describimos un algoritmo popular de _machine learning_, _K-nearest neighbors_, que se basa en la suavización de compartimiento.
...
Utilice la función `loess` para obtener un estimado uniforme del número esperado de muertes en función de la fecha. Grafique la función suave que resulta. Haga el _span_ de dos meses.

2\. Grafique los estimados suaves en función del día del año, todas en el mismo gráfico pero con diferentes colores.
...
Ajuste una línea de loess a los datos anteriores y grafique los resultados. Observe que hay poder predictivo, excepto que la probabilidad condicional no es lineal.
