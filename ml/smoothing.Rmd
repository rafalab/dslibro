...
Observen que el estimado final ahora se ve más suave.
...
[fix check sent] Una limitación del enfoque de suavización de compartimientos que acabamos de describir es que necesitamos ventanas pequeñas para mantener los supuestos aproximadamente constantes. Como resultado, terminamos con un pequeño número de puntos de datos [fix 'to average'] para promediar y obtener estimaciones imprecisas $\hat{f}(x)$. Aquí describimos cómo _la regresión ponderada local_ (loess o _local weighted regression_ en inglés) nos permite considerar tamaños de ventana más grandes. [fix check] Para hacer esto, usaremos un resultado matemático, conocido como el teorema de Taylor, que nos dice que si mira con atención a cualquier función suave $f(x)$, parecerá como una línea. Para ver por qué esto tiene sentido, consideren [fix ok?] los bordes curvos que hacen los jardineros con espadas de bordes rectos:

```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path, "garden.png"))
```

("Downing Street garden path edge"^[https://www.flickr.com/photos/49707497@N06/7361631644] por el usuario de Flckr Número 10^[https://www.flickr.com/photos/number10gov/] Licencia CC-BY 2.0^[https://creativecommons.org/licenses/by/2.0/].)
...
El valor ajustado en $x_0$ se convierte en nuestro estimado $\hat{f}(x_0)$. A continuación, mostramos [fix] el procedimiento que ocurre mientras pasamos/cambiamos/nos movemos de -155 a 0.
...
[fix check] El resultado final es un ajuste más suave que ese producido por la suavización de compartimiento porque utilizamos tamaños de muestra más grandes para estimar nuestros parámetros locales:
...
[fix] Diferentes _spans_ producen diferentes estimados. Podemos ver cómo diferentes tamaños de ventana conducen a diferentes estimados:
...
3\. `loess` tiene la opción de ajustar el modelo local[fix] _robustly_. Se implementa un algoritmo iterativo en el que, después de ajustar un modelo en una iteración, se detectan valores atípicos y se ponderan hacia abajo para la siguiente iteración. Para usar esta opción, usamos el argumento `family="symmetric"`.

### Cómo ajustar parábolas

[fix check title above ok?] El teorema de Taylor también nos dice que si miramos cualquier función matemática lo suficientemente cerca, parece una parábola. El teorema también establece que no tienen que mirar tan de cerca cuando se aproxima con parábolas como cuando se aproxima con líneas. Esto significa que podemos hacer que nuestras ventanas sean aún más grandes y ajustar parábolas en lugar de líneas.
...
`degree = 2` nos da resultados más ondulantes. Realmente preferimos `degree = 1` ya que es menos propenso a este tipo de ruido.
...
$$
con $X_1$ y $X_2$ los dos predictores definidos en la Sección \@ref(two-or-seven). [fix whole para could be reworked] En este ejemplo, los 0 y 1 que observamos son "ruidosos" porque para algunas regiones las probabilidades $p(x_1, x_2)$ no están tan cerca de 0 o 1. Por lo tanto, necesitamos estimar $p(x_1, x_2)$. La suavización es una alternativa para lograr esto. En la sección \@ref(two-or-seven) vimos que la regresión lineal no era lo suficientemente flexible como para capturar la naturaleza no lineal de $p(x_1, x_2)$; los enfoques de suavización, por ende, pueden proveer una mejora. En el siguiente capítulo describimos un algoritmo popular de _machine learning_, _K-nearest neighbors_, que se basa en la suavización de compartimiento.
...
Utilice la función `loess` para obtener un estimado uniforme del número esperado de muertes como función de la fecha. Grafique la función suave que resulta. Haga el _span_ de dos meses.

2\. [fix check creo que not ok] Grafique los estimados suaves en función del día del año, todas en el mismo gráfico pero con diferentes colores.
...
Ajuste [fix linea DE loess] una línea loess a los datos anteriores y grafique los resultados. Observe que hay poder predictivo, excepto que la probabilidad condicional no es lineal.
