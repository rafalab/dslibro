...
Los problemas de _machine learning_ a menudo implican sets de datos que son tan grandes o más grandes que el conjunto de datos MNIST. Existe una variedad de técnicas computacionales y conceptos estadísticos que son útiles para el análisis de grandes sets de datos. En este capítulo, rascamos la superficie de estas técnicas y conceptos describiendo álgebra matricial, reducción de dimensiones, regularización y factorización matricial. Utilizamos sistemas de recomendación relacionados con las clasificaciones de películas como un ejemplo motivador.

## Álgebra matricial {#matrix-algebra}

En _machine learning_, las situaciones en las que todos los predictores son numéricos, o pueden convertirse a numéricos de manera significativa, son comunes. El set de datos de dígitos es un ejemplo: cada píxel registra un número entre 0 y 255. Carguemos los datos:
...
En estos casos, a menudo es conveniente guardar los predictores en una matriz y el resultado en un vector en lugar de utilizar un _data frame_. Puede ver que los predictores se guardan como una matriz:
...
2\. ¿Algunos píxeles no son informativos? Estudien la variación de cada píxel y elimine los predictores (columnas) asociados con los píxeles que no cambian mucho y, por lo tanto, no pueden proveen mucha información para la clasificación.

3\. ¿Podemos eliminar las manchas? Primero, observen la distribución de todos los valores de píxeles. Usen esto para elegir un límite para definir el espacio no escrito. Luego, establezcan cualquier cosa por debajo de ese límite en 0.

4\. Binaricen los datos. Primero, observen la distribución de todos los valores de píxeles. Usen esto para elegir un límite para distinguir entre escribir y no escribir. Luego, conviertan todas las entradas en 1 o 0, respectivamente.
...
En álgebra matricial, tenemos tres tipos principales de objetos: escalares, vectores y matrices. Un escalar es solo un número, por ejemplo $a = 1$. Para denotar escalares en notación matricial, generalmente usamos una letra minúscula y negra.
...
Matemáticamente, los representamos con letras mayúsculas en negra:
...
Podemos llenar por fila usando el argumento `byrow`. Entonces, por ejemplo, para _transponer_ la matriz `mat`, nosotros podemos usar:
...
__Advertencia__:  La función `matrix` recicla valores en el vector **sin advertencia** si el producto de columnas y filas no coincide con la longitud del vector:

```{r}
matrix(my_vector, 4, 5)
```

Para poner las intensidades de píxeles de nuestra, digamos, tercera entrada, 
`r mnist$train$labels[3]` en la cuadrícula, podemos usar:
...
Las funciones que acabamos de describir están realizando una operación similar a la que `sapply` y la función `map` de __purrr__ hacen: aplicar la misma función a una parte de su objeto. En este caso, la función se aplica a cada fila o cada columna. La función `apply`le permite aplicar cualquier función, no solo `sum` o `mean`, a una matriz. El primer argumento es la matriz, el segundo es la dimensión, 1 para las filas, 2 para las columnas y el tercero es la función. Así por ejemplo, `rowMeans` se puede escribir como:

```{r}
avgs <- apply(x, 1, mean)
```

Pero noten que al igual que con `sapply` y `map`, podemos realizar cualquier función. Entonces, si quisiéramos la desviación estándar para cada columna, podríamos escribir:
...
Podemos hacer rápidamente un histograma de todos los valores en nuestro set de datos. Vimos cómo podemos convertir vectores en matrices. También podemos deshacer esto y convertir matrices en vectores. La operación se realizará por fila:
...
Para ver qué hace esto, miramos una matriz más pequeña:
...
Haga un diagrama de caja por clase de dígitos. Sugerencia: utilice operadores lógicos y `rowMeans`.
