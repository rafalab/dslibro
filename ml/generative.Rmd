...
Si eliminamos la constante $1/(\sqrt{2\pi} \sigma)$ y [fix ok?]luego tomamos el logaritmo, obtenemos:

$$
- \frac{(x-\mu)^2}{\sigma^2}
$$

que es el negativo de una distancia al cuadrado escalada por la desviación estándar.[fix] Para dimensiones superiores/mayores/mas elevadas, lo mismo es cierto, excepto que la escala es más compleja e implica correlaciones.
...
La exactitud es `r caret::confusionMatrix(predict(train_qda, test_set), test_set$y)$general["Precision"]`
...
Tengan en cuenta que una de las limitaciones de los modelos generativos [fix check] aquí se debe a la falta de ajuste del supuesto normal, en particular para la clase 1.
...
Los modelos generativos pueden ser muy potentes, pero solo cuando somos capaces de aproximar con éxito[fix] la distribución conjunta/común de predictores condicionados en cada clase.
...
2\. En este caso, LDA se ajusta a dos distribuciones normales de 10 dimensiones. Mire el modelo ajustado mirando el componente `finalModel` del [fix eng] resultado de `train`. Observe que hay un componente llamado `means` que incluye el estimado `means` de ambas distribuciones. Grafique [fix "mean vectors"] los vectores medios uno contra el otro y determine qué predictores (genes) parecen estar impulsando el algoritmo.


3\. Repita los ejercicios 1 con QDA.[fix ok] ¿Tiene mejor exactitud que LDA?
