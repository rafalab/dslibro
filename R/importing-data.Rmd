...
dat <- read_csv(filename)
```
Tengan en cuenta que recibimos un mensaje informándonos qué tipos de datos se utilizaron para cada columna. También observen que `dat` es un `tibble`, no solo un _data frame_. Esto es porque `read_csv` es un [fix] leedor/parser del __tidyverse__. Podemos confirmar que los datos se han leído de la siguiente manera:
...
La base R también provee funciones de importación. Estos tienen nombres similares a esas del __tidyverse__, por ejemplo `read.table`, `read.csv` y `read.delim`. Sin embargo, hay par de diferencias importantes. Para mostrar esto, leemos los datos con una función de base R:
