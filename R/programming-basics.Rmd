...
La función es particularmente útil porque sirve para vectores. Esta examina cada entrada del vector lógico y [fix] devuelve elementos del vector proporcionados en el segundo argumento, si la entrada es `TRUE`, o elementos del vector [fix] proporcionados en el tercer argumento, si la entrada es `FALSE`.
...
Una vez que comiencen a convertirse en usuarios expertos de R, es probable que necesiten cargar varios complementos de paquetes (_add-ons_ en inglés) para algunos de sus análisis. Tan pronto hagan eso, es muy posible que descubran que dos paquetes usan el mismo nombre para dos funciones diferentes. Y a menudo estas funciones hacen cosas completamente diferentes. De hecho, ya hemos visto esto porque ambos paquetes de base R  __dplyr__ y __stats__ definen una función `filter`. Hay otros cinco ejemplos en __dplyr__. Sabemos esto porque cuando cargamos __dplyr__ por primera vez, vemos el siguiente mensaje:
