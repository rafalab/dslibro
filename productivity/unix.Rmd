...
Antes de comenzar a organizar proyectos con Unix, deben elegir una convención que usarán para nombrar sistemáticamente sus archivos y directorios. Esto les ayudará a encontrar archivos y saber qué hay en ellos.
...
El comando `echo` es parecido a `cat` en R. Al ejecutar esta línea deberían ver `hello world` y luego vuelven a la línea de comando.
...
Si hacen esto correctamente, no pasará nada: no tener noticias es una buena noticia. Si el directorio ya existe, recibirán un mensaje de error y el directorio existente permanecerá intacto.
...
Nuestro directorio de trabajo ahora debería ser `~/projects`. Tengan en cuenta que en su computadora el directorio _home_ `~` se verá algo como: `/c/Users/yourusername`).

**Consejo profesional importante**: en Unix pueden completar automáticamente presionando _tab_. Esto significa que podemos escribir `cd d`, entonces presionar _tab_. Unix se completará automáticamente si `docs` es el único directorio/archivo que comienza con `d` o les mostrará las opciones. ¡Inténtelo! Usar Unix sin poder autocompletar lo hará insoportable.
...
En resumen, hemos aprendido que al usar `cd` nos quedamos un nuestra ubicación, nos movemos a un nuevo directorio usando el nombre del directorio deseado o volvemos al directorio padre usando `..`.
