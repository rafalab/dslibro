...
También hay muchos libros de referencia^[https://www.quora.com/Which-are-the-best-Unix-Linux-reference-books]. Por ejemplo, _Bite Size Linux_^[https://gumroad.com/l/bite-size-linux] y _Bite Size Command Line_^[https://jvns.ca/blog/2018/08/05/new-zine--bite-size-command-line/] son dos particularmente claros, concisos y completos.

Cuando busquen recursos de Unix, tengan en cuenta que otros términos utilizados para describir lo que aprenderemos aquí son _Linux_, el _shell_ y la _línea de comando_. Básicamente, lo que estamos aprendiendo es una serie de comandos y una forma de pensar que facilita la organización de los archivos sin usar el mouse.
...
En resumen, hemos aprendido que al usar `cd` una de tres cosas pasan: o nos quedamos en la misma ubicación, o nos movemos a un nuevo directorio usando el nombre del directorio deseado o volvemos al directorio padre usando `..`.
...
Al escribir la ruta del directorio que queremos, ya sea completa o relativa, podemos concatenar directorios con barras diagonales. Ya vimos que podemos pasar al `projects` directorio independientemente de dónde estamos escribiendo la ruta completa de esta manera:
...
Una vez que hayan dominado los conceptos básicos de Unix, deberían considerar aprender a escribir sus propios ejecutables, ya que pueden ayudar a reducir el trabajo repetitivo.
...
- _awk/sed_: estos son dos comandos muy poderosos que les permite encontrar cadenas específicas en archivos y cambiarlas.
