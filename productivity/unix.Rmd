...
También hay muchos libros de referencia^[https://www.quora.com/Which-are-the-best-Unix-Linux-reference-books]. _Bite Size Linux_^[https://gumroad.com/l/bite-size-linux] y _Bite Size Command Line_^[https://jvns.ca/blog/2018/08/05/new-zine--bite -size-command-line/] son dos ejemplos particularmente claros, concisos y completos.

Cuando busquen recursos de Unix, tengan en cuenta que otros términos utilizados para describir lo que aprenderemos aquí son _Linux_, _el shell_ y _la línea de comando_. Básicamente, lo que estamos aprendiendo es una serie de comandos y una forma de pensar que facilita la organización de los archivos sin usar el mouse.


Como motivación, vamos a comenzar a construir un directorio utilizando herramientas de Unix y RStudio.

## Convención de nomenclatura

Antes de comenzar a organizar proyectos con Unix, quieren elegir una convención que usarán para nombrar sistemáticamente sus archivos y directorios. Esto les ayudará a encontrar archivos y saber qué hay en ellos.

En general, quieren nombrar sus archivos de una manera relacionada con
sus contenidos y que especifica cómo se relacionan con otros archivos. El _Smithsonian Data Management Best Practices_^[https://library.si.edu/sites/default/files/tutorial/pdf/filenamingorganizing20180227.pdf] tienen [fix check] "cinco preceptos para el nombramiento y la organización de archivos" y son:

>> * Tener un nombre distintivo, legible por humanos que indique el contenido.
>> * Seguir un patrón consistente que sea [fix] _machine friendly_.
...
Para recomendaciones específicas, le recomendamos que siga _The Tidyverse Style Guide_^[https://style.tidyverse.org/].



## El terminal {#the-terminal}

En lugar de hacer clic, arrastrar y soltar para organizar nuestros archivos y [fix we can keep both] carpetas/ficheros, escribiremos comandos de Unix en el terminal. La forma en que hacemos esto es similar a cómo escribimos comandos en la consola R, pero en lugar de generar diagramas y resúmenes estadísticos, organizaremos archivos en nuestro sistema.
...
El comando `echo` es parecido a `cat` en R. [fix check] Al ejecutar esta línea deberían ver `hello world` y luego regresar a la línea de comando.
...
Nos referimos a todos los archivos, carpetas y programas en su computadora como el _sistema de archivos_ (_filesystem_ en inglés). Tengan en cuenta que las carpetas y los programas también son archivos, pero este es un tecnicismo que rara vez pensamos e ignoramos en este libro. Nos enfocaremos en los archivos y las carpetas por el momento y discutiremos los programas, o _ejecutables_, en una sección posterior.
...
Ahora, miren a la figura anterior que muestra un sistema de archivos. Supongan que están utilizando un sistema de apuntar y hacer clic y desea eliminar el archivo _cv.tex_. Imaginen que en su pantalla pueden ver el directorio _home_. Para borrar este archivo, deben hacer doble clic en el directorio _home_, luego _docs_, entonces _resumes_ y luego arrastrar _cv.tex_ a la papelera. Aquí están experimentando la naturaleza jerárquica del sistema: _cv.tex_ es un archivo dentro del directorio _resumes_, que es un subdirectorio dentro del directorio _docs_, que es un subdirectorio del directorio _home_.

Ahora supongan que no pueden ver su directorio _home_ en su pantalla. De alguna manera, deberían hacer que aparezca en su pantalla. Una forma de hacerlo es navegar desde lo que se llama el directorio _raíz_ (_root_ en inglés) hasta el directorio _home_. Cualquier sistema de archivos tendrá lo que se llama un directorio _raíz_, que es el directorio que contiene todos los directorios. El directorio _home_, que se muestra en la figura anterior, generalmente estará a dos o más niveles del directorio _raíz_. En Windows, tendrán una estructura como esta:
...
El concepto de una _ubicación actual_ (_current location_ en inglés) es parte de la experiencia de apuntar y hacer clic: en cualquier momento estamos _en una carpeta_ y vemos el contenido de esa carpeta. A medida que buscan un archivo, como lo hicimos anteriormente, experimentan el concepto de una ubicación actual: una vez que hagan doble clic en un directorio, cambian de ubicación y ahora están _en esa carpeta_, a diferencia de la carpeta en la que estaban antes.

En Unix, no tenemos las mismas señales visuales, pero el concepto de una _ubicación actual_ es indispensable. Nos referimos a esto como el _directorio de trabajo_ (_working directory_ en inglés). Cada ventana del terminal que tiene abierta tiene un directorio de trabajo asociado.
...
En Unix, usamos la taquigrafía `~` como un apodo para su directorio _home_. Entonces, por ejemplo, si `docs` es un directorio en su directorio _home_, la ruta completa para _docs_ puede escribirse así `~/docs`.
...
Para ver el contenido de su directorio personal, abran una terminal y escriban:
...
Pueden tratar este ejemplo en su sistema. Abran un terminal y escriban:

```{bash, eval=FALSE}
mkdir projects
```

Si hacen esto correctamente, no pasará nada: [fix check] ninguna noticia es una buena noticia. Si el directorio ya existe, recibirán un mensaje de error y el directorio existente permanecerá intacto.

Para confirmar que crearon estos directorios, puede enumerarlos:

```{bash, eval=FALSE}
ls
```
Deberían ver una lista de los directorios que acabamos de crear. Quizás también puedan ver muchos otros directorios que vienen preinstalados en sus computadoras.

Con fines ilustrativos, hagamos algunos directorios más. Pueden enumerar más de un nombre de directorio así:
...
Si cometieron un error y necesitan eliminar un directorio, pueden usar el [fix check] comando `rmdir` (_remove directory_ en inglés):
...
Esto eliminará el directorio siempre que esté vacío. Si no está vacío, recibirán un mensaje de error y el directorio permanecerá intacto. Para eliminar directorios que no están vacíos, más tarde aprenderemos sobre el comando `rm`.
...
Supongamos que abrimos un terminal y nuestro directorio de trabajo es nuestro directorio _home_. Queremos cambiar nuestro directorio de trabajo a `projects`. Hacemos esto usando el comando `cd`, que significa _cambiar directorio_ (_change directory_ en inglés):
...
Nuestro directorio de trabajo ahora debería ser `~/projects`. [fix check] Tengan en cuenta que en su computadora el directorio _home_ `~` se verá algo como `/c/Users/yourusername`).

**Consejo profesional importante**: en Unix pueden completar automáticamente presionando _tab_. Esto significa que podemos escribir `cd d`, entonces presionar _tab_. Unix se completará automáticamente si `docs` es el único directorio/archivo que comienza con `d` o les mostrará las opciones. ¡Inténtelo! [fix] Usar Unix sin poder autocompletar lo hará insoportable.

Cuando usamos `cd`, podemos escribir una ruta completa, que comenzará con `/` o `~`, o una _ruta relativa_. En el ejemplo anterior, en el que escribimos `cd projects`, usamos una ruta relativa. **Si la ruta que escriben no comienza con `/` o `~`, Unix asumirá que están escribiendo una ruta relativa, que significa que buscará el directorio en su directorio de trabajo actual**. Entonces algo como esto les dará un error:
...
El directorio de trabajo también tiene un apodo, que es un solo `.`, así que si escriben:

```{bash, eval=FALSE}
cd .
```

no se moverán. Aunque este uso particular de `.` no es útil, el apodo sí lo es a veces. Las razones no son relevantes para esta sección, pero aún deben tomar en cuenta este hecho.

En resumen, hemos aprendido [fix check] que al usar `cd` nos quedamos un nuestra ubicación, nos movemos a un nuevo directorio usando el nombre del directorio deseado o volvemos al directorio padre usando `..`.
...
Supongamos que nuestro directorio de trabajo es `~/projects` y queremos mover `figs` a `project-1`.
...
Ahora supongamos que nuestro directorio de trabajo es `~/projects` y queremos mover `reports` a `docs`, ¿cómo podemos hacer esto?
...
Si están probando esto en su sistema, [fix] recuerden usar autocompletar.

Examinemos un ejemplo más. Supongamos que estamos en `~/projects/project-1/figs` y queremos cambiar a `~/projects/project-2`. De nuevo, hay dos formas.
...
**Advertencia**: [fix check] `mv` no les preguntará "¿está seguro?" si su cambio resulta en sobrescribir un archivo.
...
Entonces en todos los ejemplos `mv` anteriores, pueden cambiar `mv` a `cp` y copiarán en lugar de mover con una excepción: no podemos copiar directorios completos sin conocer los argumentos, lo que haremos más adelante.

### `rm`: eliminar archivos

En los sistemas de apuntar y hacer clic, eliminamos los archivos arrastrándolos y soltándolos en la papelera o usando un clic especial con el mouse. En Unix, usamos el comando `rm`.
...
Deberían comenzar creando un directorio donde guardarán todos sus proyectos. Recomendamos un directorio llamado _projects_ en su directorio _home_. Para hacer esto, escribirían:
...
Nuestro proyecto se relaciona con asesinatos por armas de fuego, por lo que llamaremos al directorio de nuestro proyecto: `murders`. Será un subdirectorio en nuestros directorios de proyectos. En el directorio `murders` , crearemos dos subdirectorios para contener los datos sin procesar y los datos intermedios. Llamaremos a estos `data` y `rda`, respectivamente.

Abran una terminal y asegúresen de estar en el directorio _home_:

```{bash, eval=FALSE}
cd ~
```

Ahora ejecuten los siguientes comandos para crear la estructura de directorios que queremos. Al final, usamos `ls` y `pwd` para confirmar que hemos generado los directorios correctos en el directorio de trabajo correcto:
...
Tengan en cuenta que el camino completo de nuestro set de datos `murders` conjunto es `~/projects/murders`.
...
La mayoría de las implementaciones de Unix [fix check poderosas?] incluyen una gran cantidad de herramientas y utilidades poderosas. Acabamos de aprender los conceptos básicos aquí. Recomendamos que utilicen Unix como su herramienta principal de administración de archivos. [fix] Tomará tiempo sentirse cómodo con él, pero durante este tiempo, se encontrarán aprendiendo bastante buscando soluciones en Internet. En esta sección, superficialmentecubrimos temas ligeramente más avanzados. El objetivo principal de la sección es informarles sobre lo que está disponible en lugar de explicar todo en detalle.

### Argumentos

La mayoría de los comandos de Unix se pueden ejecutar con argumentos. Los argumentos generalmente se definen usando un guión `-` o dos guiones `--` (según el comando) seguido de una letra o una palabra. Un ejemplo de argumento es el `-r` detrás `rm`. La `r` significa recursivo y el resultado es que los archivos y directorios se eliminan recursivamente. O sea, si escriben:
...
También pueden combinar argumentos. Por ejemplo, para eliminar un directorio independientemente de los archivos protegidos, escriban:

```{bash, eval=FALSE}
rm -rf directory-name
```

Recuerden que una vez que eliminen no hay vuelta atrás.  Por lo tanto, deben usar este comando con mucho cuidado.
...
y para invertir el orden de cómo se muestran los archivos, pueden usar:
...
__Advertencia: combinando rm con el comodín `*` puede ser peligroso. Hay combinaciones de estos comandos que borrarán todo sus sistemas de archivos sin preguntar "¿está seguro?". Asegúrense de entender cómo funciona antes de usar este comodín con el comando[fix `rm`? eng too] rm.__
...
Pueden [fix todos los directorios?} verlos todos al escribir:

```{bash, eval=FALSE}
env
```

Pueden cambiar algunas de estas variables de entorno. Pero sus nombres [fix ok?] varían según las diferentes _shells_, o interfaces de órdenes de texto. Describimos los _shells_ en la siguiente sección.

### _Shells_

Mucho lo que usamos en este capítulo es parte de lo que se llama _Unix shell_. Hay varios _shells_ diferentes, aunque estas diferencias son casi imperceptibles. A la vezz, son importantes, aunque no los cubrimos aquí. Pueden ver qué _shell_ están utilizando escribiendo:
...
Una vez que sepan el _shell_, pueden cambiar las variables de entorno. En Bash Shell, lo hacemos usando `export variable value`. [fix very confusing] Para cambiar la ruta, que se describirá con más detalle pronto, escriban: (**¡No ejecuten este comando!**)

```{bash, eval=FALSE}
export PATH =/usr/bin/
```

Hay un programa que se ejecuta [fix wording] antes de que comience cada terminal donde pueden editar variables para que cambien cada vez que llame al terminal. Esto cambia en diferentes implementaciones, pero si usan bash, pueden crear un archivo llamado `.bashrc`, `.bash_profile`,`.bash_login` o `.profile`. Es posible que ya tengan uno.


### Ejecutables

En Unix, todos los programas son archivos. Se llaman _ejecutables_ (_executables_ en inglés). Entonces `ls`, `mv` y `git` son todos archivos. ¿Pero dónde están estos [fix programa o programas?] archivos de programas? Pueden averiguarlo usando el comando `which`:

```{bash}
which git
```

Ese directorio probablemente esté lleno de archivos de programas. El directorio `/usr/bin` usualmente contiene muchos archivos de programa. Si escriben:
...
[fix check] Cuando escriben `ls`, Unix sabe ejecutar un programa que es un ejecutable y que se almacena en algún otro directorio. Entonces, ¿cómo sabe Unix dónde encontrarlo? Esta información se incluye en la variable de entorno `$PATH`. Si escriben:
...
Una vez que hayan dominado los conceptos básicos de Unix, deberían considerar aprender a escribir sus propios ejecutables, ya que pueden ayudar a aliviar el trabajo repetitivo.
...
Al principio, verán una serie de símbolos como este `-rw-r--r--`. Esta cadena indica el tipo de archivo: archivo normal `-`, directorio `d` o ejecutable `x`. Esta cadena también indica el permiso del archivo: ¿es legible? [fix "writable'] ¿escribible/se puede escribir? ¿ejecutable? ¿Otros usuarios del sistema pueden leer el archivo? ¿Otros usuarios pueden editar el archivo? ¿Otros usuarios pueden ejecutar si el archivo es ejecutable? Esto es más avanzado que lo que cubrimos aquí, pero pueden aprender mucho más en un libro de referencia de Unix.
...
- _awk/sed_: estos son dos comandos muy potentes que les permite encontrar cadenas específicas en archivos y cambiarlas.


### Manipulación de archivos en R

También podemos realizar la gestión de archivos desde R. Las funciones claves para aprender se pueden ver mirando el archivo de ayuda para
