...
Para comenzar un proyecto, hagan clic en _File_ y luego en _New Project_. A menudo, ya hemos creado una carpeta para guardar el trabajo, como lo hicimos en la Sección \@ref(prep-project), y seleccionamos _Existing Directory_. Aquí les mostramos un ejemplo
...
Luego, para un proyecto de análisis de datos, generalmente seleccionan la opción _New project_:
...
Al trabajar en un proyecto, todos los archivos se guardarán y se buscarán en la carpeta asociada con el proyecto. A continuación, mostramos un ejemplo de un _script_ que escribimos y guardamos con el nombre _code.R_. Debido a que usamos un nombre significativo para el proyecto, podemos ser un poco menos informativos cuando nombramos los archivos. Aunque no lo hacemos aquí, pueden tener varios _scripts_ abiertos a la vez. Simplemente necesitan hacer clic en File_, luego en _New File_ y elegir el tipo de archivo que desean editar.
...
En la plantilla, verán varias cosas para tomar en cuenta.


### El encabezado

En la parte superior vesn
...
Estos son los fragmentos de código. Cuando compilan el documento, el código R dentro del fragmento, en este caso `summary(pressure)`, será evaluado y el resultado incluido en esa posición en el documento final.

Para añadir sus propios fragmentos R, pueden escribir los caracteres de arriba rápidamente con el comando [fix is that right?] de enlace de teclas (_key binding_ en inglés) opción-I en Mac y Ctrl-Alt-I en Windows.

Esto se aplica también a los gráficos; el gráfico se colocará en esa posición. Podemos escribir algo como esto:
...
Por defecto, el código también aparecerá. [fix seems repetetive; eng too] Para evitar que aparezca el código, pueden usar un argumento. Para evitar esto, pueden usar el argumento `echo=FALSE`. Por ejemplo:
...
Recomendamos acostumbrarse a agregar una etiqueta a los fragmentos de código R. Esto será muy útil al depurar, entre otras situaciones. Para ello, agreguen una palabra descriptiva como esta:
...
Uno de los fragmentos R contiene una llamada que parece compleja:
...
Una vez que hayan instalado los paquetes, al hacer clic en _Knit_ se compilará su archivo de R Markdown y aparecerá el documento resultante: [fix are we missing somethign here? eng too]


Esto produce un documento html que pueden ver en su directorio de trabajo. Para verlo, abran una terminal y enumeren los archivos. Pueden abrir el archivo en un navegador y usarlo para presentar su análisis. También pueden producir un documento PDF o Microsoft cambiando:
...
```
-->
Esto producirá un archivo de markdown, con sufijo `md`, que se muestra en GitHub. Debido a que hemos subido estos archivos a GitHub, pueden hacer clic en el archivo `md` y verán el informe como una página web:
...
Hay mucho más que pueden hacer con R Markdown. Le recomendamos que continúe aprendiendo a medida que adquieran más experiencia escribiendo informes en R. Hay muchos recursos gratuitos en el Internet que incluyen:

* Tutorial de RStudio: [https://rmarkdown.rstudio.com](https://rmarkdown.rstudio.com)
* La hoja de trucos: [https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf)
...
En la siguiente sección, crearemos un proyecto RStudio. En RStudio vamos a _File_ y luego a _New Project_ ...y cuando nos dan las opciones elegimos _Existing Directory_. Luego, escribimos la ruta completa del directorio `murders` directorio creado anteriormente.
...
Tengan en cuenta que cuando estemos en este proyecto, nuestro directorio de trabajo predeterminado será `~/projects/murders`. Pueden confirmar esto escribiendo `getwd()` en su sesión de R. Esto es importante porque nos ayudará a organizar el código cuando necesitemos escribir rutas de archivos.
**Consejo profesional: siempre usen rutas relativas en el código para proyectos de ciencia de datos. Estos deben ser relativos al directorio de trabajo predeterminado.** El problema con el uso de rutas completas es que es poco probable que sus códigos funcionen en sistemas de archivos distintos a los suyos, ya que las estructuras de directorio serán diferentes. Esto incluye el uso del directorio _home_ `~` como parte de su ruta.
...
Tengan en cuenta que estamos utilizando la ruta relativa `data/murders.csv`.
...
En este archivo, presentamos un comando R que no hemos visto: `save`. El comando `save` en R guarda objetos en lo que se llama un archivo _rda_: _rda_ es la abreviatura de datos de R. Recomendamos usar el sufijo `.rda` en archivos que guardan objetos R. Veran que `.RData` también se utiliza.
...
Ahora supongamos que queremos guardar el gráfico generado para usarlo en un informe o presentación. Podemos hacer esto con el comando `ggsave` de __ggplot__. ¿Pero dónde ponemos el gráfico? Deberíamos organizarnos sistemáticamente para que podamos guardar los gráficos en un directorio llamado `figs`. Comiencen creando un directorio escribiendo lo siguiente en la terminal:
...
a su _script_ de R. Si ejecutan el _script_ ahora, se guardará en un archivo png en el directorio `figs`. Si quisiéramos copiar ese archivo en otro directorio donde estamos desarrollando una presentación, podemos evitar usar el mouse usando el comando `cp` en nuestro terminal.

### Agregar un archivo README

Ahora tienen un análisis autónomo en un directorio. Una recomendación final es crear un archivo `README.txt` que describe lo que cada uno de estos archivos hace en beneficio de otros que leen su código, incluyedo ustedes en el futuro. Esto no sería un _script_, sino solo algunas notas. Una de las opciones proporcionadas al abrir un nuevo archivo en RStudio es un archivo de texto. Puede guardar algo como esto en el archivo de texto:
