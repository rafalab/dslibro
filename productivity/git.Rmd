...
1. Compartir: Incluso si no aprovechamos la funcionalidad avanzada y poderosa de control de versiones, aún podemos usar Git y GitHub para compartir nuestro código. 
...
Quieren escoger un nombre con cuidado. Debe ser corto, fácil de recordar y deletrear,  relacionado de alguna manera con su nombre y, claro, profesional. Este último es importante ya que podrían estar enviando enlaces a sus cuentas de GitHub a posibles empleadores. En el siguiente ejemplo, sacrifico la facilidad de deletreo para incorporar mi nombre. Sus iniciales y apellidos suelen ser una buena opción. Si tienen un nombre muy común, es posible que tengan que tomar eso en cuenta. Una solución sencilla sería agregar números o deletrear parte de su nombre.
...
El primer paso para crear un repositorio para su código es inicializar en GitHub. Como ya crearon una cuenta, tendrán una página en GitHub con la URL `http://github.com/username`.

Para crear un repositorio, primero inicien una sesión en sus cuentas haciendo clic en el botón _Sign In_ en [https://github.com](http://github.com). Es posible que ya hayan iniciado una sesión, en cuyo caso el botón _Sign In_ no aparecerá.
<!--
...
También estamos agregando un archivo temporal que no queremos rastrear:
...
Sin embargo, si volvemos a editar ese archivo, solo cambia en el directorio de trabajo. Para agregar al repositorio local, necesitamos _stage it_ y _commit_ los cambios que se agregan al repositorio local:
...
Sin embargo, en este ejemplo particular, no podrán hacerlo porque no tiene permiso para editar el _Upstream Repository_. Si este fuera su repositorio, podrían.
...
Y entonces para hacer estas copias al _Staging Area_ y _Working Directory_, utilizamos el comando:
...
Aprenderemos en la Sección \@ref(rstudio-git) cómo RStudio tiene botones para hacer todo esto. Los detalles ofrecidos aquí les ayuda a entender lo que sucede en el fondo.
...
Supongan que ya tenemos un directorio local poblado y queremos convertir este directorio en un repositorio colaborativo de GitHub. La forma más eficiente de lograr esto es _inicializando_ el directorio local.
...
Si seleccionan esta pestaña, les mostrará los archivos de sus proyectos con algunos iconos que le brindan información sobre estos archivos y su relación con el repositorio. En el ejemplo a continuación, ya agregamos un archivo a la carpeta, llamado _code.R_, que pueden ver en el [fix "editing pane"] panel de edición.
