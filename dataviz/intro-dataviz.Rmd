# (PART) Visualización de datos EN CONSTRUCCION {-}
...
Esto nos recuerda del dicho "una imagen vale más que mil palabras". La visualización de datos ofrece una forma poderosa de comunicar hallazgos basados en datos. En algunos casos, la visualización es tan convincente que no se requiere un análisis de seguimiento.

La creciente disponibilidad de sets de datos informativos y de herramientas de _software_ ha llevado a una mayor dependencia de la visualizacion de datos en muchas industrias, academias y gobiernos. Un ejemplo destacado son las organizaciones de noticias, que están adoptando cada vez más el _periodismo de datos_ e incluyendo _infografías_[fix] efectivas/eficaces como parte de sus informes.

Un ejemplo particularmente efectivo es un artículo del Wall Street Journal^[http://graphics.wsj.com/infectious-diseases-and-vaccines/?mc_cid=711ddeb86e] que muestra los datos relacionados con el impacto de las vacunas en la lucha contra las enfermedades infecciosas. Uno de los gráficos muestra los casos de sarampión por estado de EE. UU. a lo largo de los años con una línea vertical que demuestra cuándo se introdujo la vacuna.
...
Otro ejemplo sorprendente proviene de un gráfico del New York Times^[http://graphics8.nytimes.com/images/2011/02/19/nyregion/19schoolsch/19schoolsch-popup.gif], que resume[fix] los puntajes de los exámenes de los Regentes de la ciudad de Nueva York. Según el artículo^[https://www.nytimes.com/2011/02/19/nyregion/19schools.html], estos puntajes se recopilan por varias razones, incluso para determinar si un estudiante se gradúa de la escuela secundaria. En la ciudad de Nueva York,  se necesita un puntaje mínimo de 65 para aprobar. La distribución de los puntajes de las pruebas nos obliga a notar algo un poco problemático:
...
El puntaje de prueba más común es la calificación mínima para aprobar, con muy pocos puntajes justo por debajo del umbral. [fix] Este resultado inesperado es consistente con el aumento de la puntuación de los estudiantes cerca de aprobar, pero sin el mínimo de 65.

Este es un ejemplo de cómo la visualización de datos puede conducir a descubrimientos que de otro modo se perderían si simplemente sometiéramos los datos a una batería de herramientas o procedimientos de análisis de datos. La visualización de datos es la herramienta más sólida de lo que llamamos [fix] _análisis de datos exploratorios_ (o EDA por sus siglas en inglés). John W. Tukey^[https://en.wikipedia.org/wiki/John_Tukey], considerado el padre de EDA, una vez dijo:

>> "El mayor valor de una imagen es cuando nos obliga a notar lo que nunca
esperábamos ver."

Muchas herramientas ampliamente utilizadas de análisis de datos fueron iniciadas por descubrimientos realizados a través de EDA. Este es quizás la parte más importante del análisis de datos, pero a menudo se ignora.

La visualización de datos ahora también es omnipresente en organizaciones filantrópicas y educativas. En las conferencias "Nuevas ideas sobre la pobreza"^[https://www.ted.com/talks/hans_rosling_reveals_new_insights_on_poverty?language=en] y "Las mejores estadísticas que has visto"^[https://www.ted.com/talks/ hans_rosling_shows_the_best_stats_you_ve_ever_seen], Hans Rosling nos obliga a notar lo inesperado con una serie de tramas relacionadas con la salud y la economía mundial. En sus videos, Rosling usa gráficos animados para mostrarnos cómo está cambiando el mundo y cómo las viejas narraciones ya no son ciertas.
...
También es importante tener en cuenta que los errores/equivocaciones/fallas/descuidos, [fix] sesgos/prejuicios, errores sistemáticos y otros problemas inesperados a menudo conducen a datos que deben manejarse con cuidado. No descubrir estos problemas puede dar lugar a análisis defectuosos y descubrimientos falsos. Como ejemplo, consideren que [fix] los instrumentos/aparatos.dispositivos de medición/measurement devices a veces fallan y que la mayoría de los procedimientos de análisis de datos no están diseñados para detectarlos.
...
En esta parte del libro, aprenderemos los conceptos básicos de visualización de datos y de análisis exploratorio de datos mediante el uso de tres ejemplos motivantes. Usaremos el paquete __ggplot2__ para codificar. Para aprender los conceptos básicos, comenzaremos con un ejemplo algo artificial: alturas [fix]reportadas por estudiantes. Luego cubriremos dos ejemplos mencionados anteriormente: 1) la salud y la economía mundial y 2) las tendencias de las enfermedades infecciosas en Estados Unidos.

Por supuesto, la visualización de datos es mucho más de lo que cubrimos aquí. Aquí ofrecemos unas referencias para los que quieran aprender más:
...
Además, no cubrimos gráficos interactivos, un tema demasiado avanzado para este libro. Algunos recursos útiles para aquellos interesados en aprender más se encuentran a continuación:
