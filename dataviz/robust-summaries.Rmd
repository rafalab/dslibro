...
Nuestro colega utiliza el hecho de que las alturas suelen estar bien aproximadas por una distribución normal y [fix] resume los datos con el promedio y la desviación estándar:
...
Si queremos que un valor atípico sea más raro, podemos aumentar el 1.5 a un número mayor. Tukey también usó 3 y los denominó _far out outliers_ [fix] o _valores atípicos extremos_. Con una distribución normal,
...
## [fix] Mediana desviación absoluta

Otra forma de estimar de manera[fix] _robustly_ sólida la desviación estándar en presencia de valores atípicos es usar la desviación absoluta media o MAD (_median absolute deviataion_) por sus siglas en inglés. Para calcular el MAD, primero calculamos la mediana y luego, para cada valor, calculamos la distancia entre ese valor y la mediana. El MAD se define como la mediana de estas distancias. Por razones técnicas no que no discutimos aquí, esta cantidad debe multiplicarse por 1.4826 para asegurar que se aproxima a la desviación estándar real. La función  `mad` ya incorpora esta corrección. Para los datos de altura, obtenemos una MAD de:
...
## Estudio de caso: [fix] alturas de estudiantes autoinformadas
...
Podemos revisar todas las respuestas sin sentido observando los datos que Tukey[fix] considera _far out_ o fuera de lugar:
