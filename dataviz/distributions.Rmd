...
Para entender las densidades suaves, tenemos que entender[fix] las _estimaciones/estimados_, un tema que no abordamos hasta más tarde. Sin embargo, ofrecemos una explicación heurística para ayudarles a entender los conceptos básicos y así poder utilizar esta útil herramienta de visualización de datos.
...
[fix all]Sin embargo, hacemos una suposición que quizás nos ayuda a aproximarnos. Si tuviéramos 1,000,000 de valores, medidos con mucha precisión, podríamos hacer un histograma con [fix] contenedores/bins muy, muy pequeños. La suposición es que si mostramos esto, la altura de los [fix] contenedores consecutivos será similar. Esto es lo que queremos decir con suave: no tenemos grandes saltos en las alturas de los contenedores consecutivos. A continuación tenemos un histograma hipotético con contenedores de tamaño 1:
...
Entre más pequeños hacemos los contenedores, más suave se vuelve el histograma. Aquí están los histogramas con ancho de bin de 1, 0.5 y 0.1:
...
La densidad suave es básicamente la curva que atraviesa la parte superior de las barras del histograma cuando los contenedores son muy, muy pequeños. Para que la curva no dependa del tamaño hipotético de la lista hipotética, calculamos la curva [fix] en/usando? frecuencias en lugar de conteos:
...
Aquí hay un gráfico de la densidad suave y de la distribución normal con promedio = `r round(m,1)` y SD = `r round(s,1)` trazado como una línea negra con la densidad suave de nuestras alturas de estudiantes en azul:
