# [fix] Cómo visualizar las distribuciones de datos {#distributions}
...
### [fix]Densidades permiten la estratificación
...
b. 95%
c. 80%
d. [fix eng?] 72 pulgadas


4\. A la pulgada más cercana, ¿qué altura `m` tiene la propiedad de que la mitad de los estudiantes varones son más altos que `m` y la mitdad son más cortos?
...
b. La mayoría de los estados tienen tasas de homicidio por [fix] debajo/inferior a de 2 por 100,000.
c. Todos los estados tienen tasas de asesinatos superiores a 2 por 100,000.
d. Con la excepción de 4 estados, las tasas de asesinatos son [fix] inferiores/menos que a 5 por cada 100,000.
...
10\. Abajo hay tres gráficos de densidad. [fix] ¿Es posible que sean del mismo set de datos?
...
La distribución normal, también conocida como curva de campana y distribución Gausiana, es uno de los conceptos matemáticos más famosos de la historia. Una razón para esto es que se producen distribuciones aproximadamente normales en muchas situaciones, incluyendo las ganancias de juego, las alturas, los pesos, la presión arterial, la puntuación de las pruebas estandarizadas y [fix] los errores de medición experimentales/"experimental measurement errors". Hay explicaciones para esto y las describiremos más adelante. Aquí nos enfocamos en cómo la distribución normal nos ayuda a resumir los datos.
...
Entonces se pueden usar las funciones preconstruidas `mean` y `sd` (tengan en cuenta que por razones que se explican [fix eng too]en la Sección \@ref(data-driven-model), `sd` se divide por `length(x)-1` más bien que `length(x)`) :
```{r}
m <- mean(x)
s <- sd(x)
c(average = m, sd = s)
...
Para los datos que se distribuyen aproximadamente normalmente, es conveniente pensar en términos de _unidades estándar_. La unidad estándar de un valor nos dice cuántas desviaciones estándar se alejan del promedio. Específicamente, [fix] para/por un valor `x` de un vector `X`, definimos el valor de `x` en unidades estándar como `z = (x - m)/s` con `m` y `s` el promedio y la desviación estándar de `X`, respectivamente. ¿Por qué es esto conveniente?
...
¡La proporción es aproximadamente el 95%, que es lo que predice la distribución normal! [fix] Para tener hasta más confirmación de que la aproximación es precisa, podemos usar gráficos Q-Q (_quantile-quantile_ plots en inglés).
...
2. [fix] Definir un vector de cuantiles $q_1, \dots, q_m$ para sus datos para las proporciones $p_1, \dots, p_m$. Nos referimos a estos como los _cuantiles de muestra_ (_sample quantiles_ en inglés).
...
Mientras que para la ilustración anterior usamos 20 cuantiles, el valor por defecto de la función `geom_qq` es utilizar [fix] la misma cantidad de cuantiles como puntos de datos.

## [fix] Percentiles/Porcentajes?

Antes de continuar, definamos algunos términos que se usan comúnmente en el análisis exploratorio de datos.

_Percentiles_ son casos especiales de _cuantiles_ que se usan comúnmente. Los percentiles son los cuantiles que obtienen al configurar el $p$ a $0.01, 0.02, ..., 0.99$. [fix] Denominamos, por ejemplo, el caso de $p=0.25$ el percentil 25, ya que nos da un número para el cual el 25% de los datos está debajo. El percentil más famoso es el 50, también conocido como la _mediana_ (_median_ en inglés).
...
Supongamos que queremos resumir la distribución de la tasa de asesinatos. Usando la técnica de visualización de datos que hemos aprendido, [fix] podemos vemos que la aproximación normal no aplica aquí:
...
con el cuadro definido por el percentil 25% y 75% y los bigotes mostrando el rango. La distancia entre estos dos se llama el rango _intercuartil_. Los dos puntos son valores atípicos según la definición de Tukey. La mediana se muestra con una línea horizontal. Hoy en día, denominamos estos _diagramas de caja_ (_boxplots_ en inglés).
