...
Para generar un diagrama de barras podemos usar el `geom_bar` geometría. El valor predeterminado es contar el número de cada categoría y dibujar una barra. Aquí está el gráfico para las regiones de los Estados Unidos.
...
Para crear una densidad suave, usamos el `geom_density`. Para hacer un gráfico de densidad uniforme con los datos mostrados previamente como un histograma, podemos usar este código:
...
Para gráficos Q-Q usamos el `geom_qq` geometría. Del archivo de ayuda, aprendemos que necesitamos especificar el `sample` (aprenderemos sobre muestras en un capítulo posterior). Aquí está el gráfico Q-Q para [fix] alturas de varones/hombres/men heights.
...
Para hacer un gráfico Q-Q rápido tienes que usar el `sample` argumento. Tenga en cuenta que podemos agregar capas tal como lo hacemos con `ggplot`.
...
Utilizar el `binwidth` argumento para cambiar el histograma realizado en el ejercicio anterior para usar compartimientos de tamaño 1 pulgada.
