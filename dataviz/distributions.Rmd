...
Para generar un diagrama de barras (_barplots_ en inglés) podemos usar la geometría `geom_bar`. [fix] "The default is to"/La acción por defecto de R? es contar el número de cada categoría y dibujar una barra. Aquí está el diagrama de barras para las regiones de Estados Unidos.
...
Para generar histogramas utilizamos `geom_histogram`. Al revisar el archivo de ayuda para esta función, vemos que el único argumento requerido es `x`, la variable para la cual construiremos un histograma. [fix] We dropped the/Ignoramos la `x` porque sabemos que es el primer argumento.
El código se ve así:
...
Para cambiar la suavidad de la densidad, utilizamos el argumento `adjust` para multiplicar el valor por defecto por ese `adjust`. Por ejemplo, si queremos que el [fix] parámetro de suavizado (_bandwith_ en inglés) sea el doble de grande, usamos:
...
Para gráficos Q-Q usamos la geometría `geom_qq`. Del archivo de ayuda, aprendemos que necesitamos especificar el `sample` (aprenderemos sobre _samples_ en un capítulo posterior). Aquí está el gráfico Q-Q para alturas de varones:
...
**Nota técnica**: [fix eng too] La razón por la que usamos `I("black")` es porque queremos que `qplot` trate `"black"` como un carácter en lugar de convertirlo en un factor, que es el comportamiento por defecto dentro de `aes`, que se llama internamente aquí. En general, la función `I` se usa en R para decir "manténgalo como está".
