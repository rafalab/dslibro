...
Para usar __ggplot2__ tendrán que aprender varias funciones y argumentos. Estos son difíciles de memorizar, por lo que les recomendamos que tengan a mano la hoja de referencia de ggplot2. Pueden obtener una copia aquí: [https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdffont>(https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf) o simplemente realizando una búsqueda en internet de "ggplot2 cheat sheet".
...
Esta visualización de datos nos muestra prácticamente toda la información de la tabla de datos. El código necesario para hacer este gráfico es relativamente sencillo. Aprenderemos a crear el gráfico parte por parte.
...
* __Geometría__: El gráfico de arriba es un diagrama de dispersión. Esto se denomina el componente de __geometría__ . Otras posibles geometrías son diagrama de barras, histograma, densidades suaves (_smooth densities_ en inglés), diagrama qq y diagrama de caja. 
...
En `ggplot2` creamos gráficos agregando _capas_ (_layers_ en inglés). Las capas pueden definir geometrías, calcular estadísticas de resumen, definir qué escalas (_scales_ en inglés), o incluso cambiar estilos. Para agregar capas, usamos el símbolo `+`. En general, una línea de código se verá así:
...
Para que `geom_point` funcione bien, necesitamos proveer datos y una correspondencia. Ya hemos conectado el objeto `p` con la tabla de datos `murders`, y si agregamos la capa `geom_point`, está por defecto usa estos datos. Para saber qué correspondencias se esperan, lean la sección __Aesthetics__ del archivo de ayuda `geom_point`:
...
Cada función de geometría tiene muchos argumentos además de `aes` y `data`. Estos tienden a ser específicos a la función. Por ejemplo, en el gráfico que queremos hacer, los puntos son más grandes que el tamaño predeterminado. En el archivo de ayuda vemos que `size` es una estética y podemos cambiarla así:
...
En la anterior línea de código, definimos el mapeo `aes(population/10^6, total)` dos veces, una vez en cada geometría. Podemos evitar esto usando un mapeo estético _global_ cuando definimos la pizarra en blanco dada por el objeto `ggplot`. Recuerden que la función `ggplot` contiene un argumento que nos permite definir mapeos estéticas:
...
Los mapeos `x` y `y` se heredan de esos ya definidos en `p`, así que no los redefinimos. También movemos `aes` al primer argumento, ya que es donde se esperan los mapeos en esta llamada.

Aquí vemos otro comportamiento útil por defecto: __ggplot2__ automáticamente agrega una leyenda que asigna el color a la región. Para evitar agregar esta leyenda, establecemos el argumento `geom_point` como `show.legend = FALSE`.
...
El paquete __ggthemes__ agrega muchos otros temas. Entre esos están el tema `theme_economist` que elegimos. Después de instalar el paquete, pueden cambiar el estilo agregando una capa como esta:
