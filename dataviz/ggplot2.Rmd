...
En la línea de código anterior, definimos el mapeo `aes(population/10^6, total)` dos veces, una vez en cada geometría. Podemos evitar esto usando un mapeo estético _global_ cuando definimos la pizarra en blanco dada por el objeto `ggplot`. Recuerden que la función `ggplot` contiene un argumento que nos permite definir asignaciones estéticas:
...
Debido a que ahora estamos en la escala logarítmica, el ajuste a la posición (_nudge_ en inglés), debe hacerse más pequeño.
...
y luego probaremos lo que sucede agregando diferentes llamadas a `geom_point`. Por ejemplo, podemos hacer que todos los puntos sean azules agregando el argumento `color`:
...
Los mapeos`x` y `y` se heredan de esas ya definidas en `p`, así que no los redefinimos. También movemos `aes` al primer argumento, ya que es donde se esperan los mapeos en esta llamada.

Aquí vemos otro comportamiento útil por defecto: __ggplot2__ agrega automáticamente una leyenda que asigna el color a la región. Para evitar agregar esta leyenda, establecemos el argumento `geom_point` como `show.legend = FALSE`.
...
Para agregar una línea usamos la función `geom_abline`. __ggplot2__ utiliza `ab` en el nombre para recordarnos que estamos suministrando el intercepto (`a`) y pendiente (`b`). La línea predeterminada tiene pendiente 1 e intercepto 0, por lo que solo tenemos que definir el intercepto:
...
La diferencia final tiene que ver con la posición de las etiquetas. En nuestro gráfico, algunas de las etiquetas se superponen. El paquete de complementos __ggrepel__ incluye una geometría que agrega etiquetas a la vez que garantiza que no se superpongan entre sí. Simplemente cambiamos `geom_text` a `geom_text_repel`.
...
Esto parece demasiado código para una gráfico tan sencillo. La función `qplot` sacrifica la flexibilidad proporcionada por el enfoque de `ggplot`, pero nos permite generar un gráfico rápidamente.
...
a. Necesitamos mapear un caracter a cada punto a través del argumento de etiqueta en `aes`.
