...
Construiremos un gráfico que resume el set de datos de asesinatos de Estados Unidos que se vea así:
...
Podemos ver claramente cuánto varían los estados según el tamaño de la población y el número total de asesinatos. No es sorprendente que también veamos una relación clara entre los totales de asesinatos y el tamaño de la población. Un estado que cae en la[fix]dashed grey line/ línea discontinua gris tiene la misma tasa de asesinatos que el promedio de EE. UU. Las cuatro regiones geográficas se denotan con color [fix]; que resaltan desmuestran cómo la mayoría de los estados del sur tienen tasas de asesinatos por encima del promedio.
...
* __Geometría__: El diagrama de arriba es un diagrama de dispersión. Esto se denomina el componente de __geometría__ . Otras posibles geometrías son diagrama de barras, histograma,[fix] smooth densities/densidades suaves, diagrama qq y diagrama de caja. Aprenderemos más sobre esto en la parte del libro "Visualización de datos". [aren't we in that part?]
* [fix] __Mapeo estético__: El gráfico usa varias señales visuales para representar la información proporcionada por el set de datos. Las dos señales más importantes en este gráfico son las posiciones de los puntos en el eje X y el eje Y, que representan el tamaño de la población y el número total de asesinatos, respectivamente. Cada punto representa una observación diferente, y [fix] we map/definimos una correspondencia entre los datos sobre estas observaciones y señales visuales como las escalas X e Y. El color es otra señal visual que asignamos a la región. Nos referimos a esto como el componente __mapeo estético__. La forma en que definimos la correspondencia depende de qué __geometría__ estamos usando.
...
En `ggplot2` creamos gráficos agregando [fix - capas?] _layers_. Las capas pueden definir geometrías, calcular estadísticas de resumen, definir qué escalas usar, o incluso cambiar estilos.
...
Para que `geom_point` funcione correctamente, necesitamos proveer datos y una correspondencia. Ya hemos conectado el objeto `p` con la table de datos `murders`, y si agregamos la capa `geom_point` por defecto usa estos datos. Para saber qué correspondencias se esperan, leemos la sección __Aesthetics__ del archivo de ayuda `geom_point` archivo de ayuda:
...
Una segunda capa en el diagrama que deseamos hacer implica agregar una etiqueta a cada punto para identificar el estado. los `geom_label` y `geom_text` las funciones nos permiten agregar texto al gráfico con y sin un rectángulo detrás del texto, respectivamente.
...
La función `qplot` sacrifica la flexibilidad proporcionada por el `ggplot` enfoque, pero nos permite generar una trama rápidamente.
...
a. No pasa nada.
b. Una trama de pizarra en blanco.
