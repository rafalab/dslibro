...
Cuando se usan diagramas de barras, es erróneo no comenzar las barras en 0. Esto se debe a que, al usar un diagrama de barras, estamos implicando que la longitud es proporcional a las cantidades que se muestran. Al evitar 0, se pueden hacer diferencias relativamente pequeñas verse mucho más grandes de lo que realmente son. Este acercamiento a menudo es utilizado por políticos u [fix] organizaciones de medios de comunicación que intentan exagerar la diferencia. [fix] A continuación se muestra un ejemplo ilustrativo utilizado por Peter Aldhous en esta conferencia: http://paldhous.github.io/ucb/2016/dataviz/week2.htmlfont^[http://paldhous.github.io/ucb/2016/dataviz/week2.html].
...
Abajo vemos otro ejemplo, que se describe en detalle en una artículo del blog "Flowing Data":
...
Este gráfico hace que un aumento del 13% parezca [fix] un cambio de cinco veces/"five fold change". Aquí tenemos el gráfico apropiado:
...
Finalmente, aquí tenemos un ejemplo extremo que hace que una diferencia muy pequeña, de menos de 2%, [fix] parezca un cambio de 10 a 100 veces:
...
Cuando se usa posición en lugar de longitud, no es necesario incluir 0. Este es  el caso en particular cuando queremos comparar las diferencias entre los grupos en relación con la variabilidad dentro de un grupo. [fix] Aquí tenemos un ejemplo ilustrativo que muestra la esperanza de vida promedio de país estratificada en todos los continentes en 2012:
...
Cuando uno de los ejes se usa para mostrar categorías, como se hace en los diagramas de barras, el comportamiento por defecto de __ggplot2__ es ordenar las categorías alfabéticamente cuando se definen por cadenas de caracteres. Si están definidas por factores, se ordenan según los niveles de factores. Raramente queremos usar el orden alfabético. En cambio, debemos ordenar por una cantidad significativa. En todos los casos anteriores, los diagramas de barras se ordenaron según los valores que mostraban. La excepción fueron los diagramas de barras comparando navegadores. En ese caso, mantuvimos el orden igual en todos los diagramas de barras para facilitar la comparación. Específicamente, en lugar de ordenar los navegadores por separado en los dos años, ordenamos ambos años por el valor promedio de 2000 y 2015.
...
Mirando este gráfico, uno concluiría que los países de Asia son mucho más poblados que los de otros continentes. Siguiendo el principio de "mostrar los datos", notamos rápidamente que esto se debe a dos países muy grandes, que suponemos son India y China:
...
Otras transformaciones que deben considerar son la transformación logística (`logit`), que es útil para mejor ver los cambios en las probabilidades, y la transformación de la raíz cuadrada (`sqrt`), que es útil para conteos.
...
No hay geometría para los _slope charts_ en __ggplot2__, pero podemos construir una usando `geom_line`. Necesitamos hacer algunos ajustes para agregar etiquetas. A continuación se muestra un ejemplo que compara 2010 a 2015 para los grandes países occidentales:
...
Una ventaja del _slope chart_ es que rápidamente nos permite tener una idea de los cambios basados en la pendiente de las líneas. Aunque estamos usando el ángulo como señal visual, también tenemos una posición para determinar los valores exactos. Comparar las mejoras es un poco más difícil con un diagrama de dispersión:
...
En el diagrama de dispersión, hemos seguido el principio _usar ejes comunes_ ya que estamos comparando estos antes y después. Sin embargo, si tenemos muchos puntos, los _slope charts_ dejan de ser útiles ya que se hace difícil ver todas las líneas.
