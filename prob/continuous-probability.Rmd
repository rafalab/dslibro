...
6\. [fix check] Para ver las matemáticas que explican por qué las respuestas a las preguntas 3, 4 y 5 son las mismas, suponga que tiene una variable aleatoria con promedio $m$ y error estándar $s$. Suponga que quiere saber la probabilidad de $X$ ser más pequeño o igual a $a$. Recuerda que, por definición, $a$ es $(a - m)/s$ desviaciones estandar $s$ lejos del promedio $m$. La probabilidad es:
...
Entonces, sin importar las unidades, la probabilidad de $X\leq a$ es igual a la probabilidad de que una variable normal estándar sea menor que $(a - m)/s$. ¿Si `mu` es el promedio y `sigma` el error estándar, cuál de los siguientes códigos de R le dará la respuesta correcta en cada situación?
