...
$$
[fix] Y recuerden $\mbox{E}[S]$ y $\mbox{SE}[S]$ son el valor esperado y el error estándar de $S$, respectivamente. Todo lo que hicimos arriba fue sumar y dividir por la misma cantidad en ambos lados. Hicimos esto porque ahora el término de la izquierda es una variable aleatoria normal estándar, a la que cambiaremos el nombre $Z$.
...
Ahora, debido a que Z es un [fix] azar normal con valor esperado 0 y error estándar 1, significa que la cantidad en el lado derecho del [fix] signo < debe ser igual a:

```{r}
qnorm(0.01)
```

para que la ecuación sea [fix] verdadera/cierta. Recuérdalo $z=$ `qnorm(0.01)` nos da el valor de $z$ para cual:
...
[fix] nos beneficiaremos. Al 5%, tenemos garantizado un valor positivo esperado de:
...
d. El número de simulaciones de Monte Carlo.
