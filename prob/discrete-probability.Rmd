...
Comenzamos cubriendo algunos principios básicos relacionados con los datos categóricos. El subconjunto de probabilidad se conoce como _discreta probabilidad_. Nos ayudará a comprender la teoría de probabilidad que luego presentaremos para datos numéricos y continuos, que es mucho más común en las aplicaciones de ciencia de datos. La probabilidad discreta es más útil en los juegos de cartas y, por lo tanto, los usamos como ejemplos.

### Frecuencia relativa

La palabra probabilidad se usa en el lenguaje cotidiano. Con frecuencia, responder preguntas sobre la probabilidad es difícil, si no imposible. Aquí discutimos una definición matemática de _probabilidad_ que nos permite dar respuestas precisas a ciertas preguntas.

Por ejemplo, si tengo 2 cuentas rojas y 3 cuentas azules dentro de una urna^[https://en.wikipedia.org/wiki/Urn_problem] (la mayoría de los libros de probabilidad usan este término arcaico, entonces nosotros también) y elijo uno al azar, ¿cuál es la probabilidad de elegir una roja? Nuestra intuición nos dice que la respuesta es 2/5 o 40%. Se puede dar una definición precisa al señalar que hay cinco resultados posibles de los cuales dos satisfacen la condición necesaria para el evento "recoger una cuenta roja". Dado que cada uno de los cinco resultados tiene la misma posibilidad de ocurrir, concluimos que la probabilidad es .4 para rojo y .6 para azul.
...
Usamos la notación $\mbox{Pr}(A)$ para denotar la probabilidad de evento $A$ sucediendo. Usamos el término muy general _event_ para referirnos a cosas que pueden suceder cuando algo ocurre por casualidad. En nuestro ejemplo anterior, el evento fue "elegir una cuenta roja". En una encuesta política en la que llamamos al azar a 100 votantes probables, un ejemplo de un evento es "llamar a 48 demócratas y 52 republicanos".

En aplicaciones de ciencia de datos, a menudo trataremos con variables continuas. Estos eventos a menudo serán cosas como "es esta persona más alta que 6 pies". En este caso, escribimos eventos en una forma más matemática: $X \geq 6$. Veremos más de estos ejemplos más adelante. Aquí nos centramos en datos categóricos.


### Distribuciones de probabilidad

Si conocemos la frecuencia relativa de las diferentes categorías, definir una distribución para resultados categóricos es relativamente sencillo. Simplemente asignamos una probabilidad a cada categoría. En los casos que pueden considerarse como cuentas en una urna, para cada tipo de cuenta, su proporción define la distribución.

Si estamos llamando al azar a votantes probables de una población que es 44% demócrata, 44% republicana, 10% indecisa y 2% del Partido Verde, estas proporciones definen la probabilidad para cada grupo. La distribución de probabilidad es:
...
## Simulaciones de Monte Carlo para datos categóricos

Las computadoras proporcionan una forma de realizar el experimento aleatorio simple descrito anteriormente: elija una cuenta al azar de una bolsa que contenga tres cuentas azules y dos rojas. Los generadores de números aleatorios nos permiten imitar el proceso de selección al azar.

Un ejemplo es el `sample` funcionan en R. Demostramos su uso en el siguiente código. Primero, usamos la función `rep` para generar la urna:
...
y luego usar `sample` elegir una cuenta al azar:

```{r}
sample(beads, 1)
```

Esta línea de código produce un resultado aleatorio. Queremos repetir este experimento un número infinito de veces, pero es imposible repetirlo para siempre. En cambio, repetimos el experimento un número suficientemente grande de veces para que los resultados sean prácticamente equivalentes a repetirse para siempre. **Este es un ejemplo de una simulación _Monte Carlo_**.

Gran parte de lo que estudian los estadísticos matemáticos y teóricos, que no cubrimos en este libro, se relaciona con proporcionar definiciones rigurosas de "prácticamente equivalente", así como estudiar qué tan cerca nos lleva un gran número de experimentos a lo que sucede en el límite. Más adelante en esta sección, proporcionamos un enfoque práctico para decidir qué es "lo suficientemente grande".

Para realizar nuestra primera simulación de Monte Carlo, utilizamos el `replicate` función, que nos permite repetir la misma tarea varias veces. Aquí, repetimos el evento aleatorio $B =$ 10,000 veces:
...
Los números anteriores son las probabilidades estimadas proporcionadas por esta simulación de Monte Carlo. La teoría estadística, no cubierta aquí, nos dice que como $B$ se hace más grande, las estimaciones se acercan a 3/5 = .6 y 2/5 = .4.

Aunque este es un ejemplo simple y no muy útil, utilizaremos las simulaciones de Monte Carlo para estimar las probabilidades en los casos en que sea más difícil calcular las exactas. Antes de profundizar en ejemplos más complejos, usamos los simples para demostrar las herramientas informáticas disponibles en R.
...
A lo largo de este libro, utilizamos generadores de números aleatorios. Esto implica que muchos de los resultados presentados pueden cambiar por casualidad, lo que sugiere que una versión congelada del libro puede mostrar un resultado diferente al que obtiene cuando intenta codificar como se muestra en el libro. Esto está bien, ya que los resultados son aleatorios y cambian de vez en cuando. Sin embargo, si desea asegurarse de que los resultados sean exactamente los mismos cada vez que los ejecute, puede establecer la semilla de generación de números aleatorios de R en un número específico. Arriba lo establecemos en 1986. Queremos evitar usar la misma semilla cada vez. Una forma popular de recoger la semilla es el año, mes y día. Por ejemplo, elegimos 1986 el 20 de diciembre de 2018: $2018 - 12 - 20 = 1986$.

Puede obtener más información sobre cómo configurar la semilla mirando la documentación:

```{r,eval=FALSE}
?set.seed
```

En los ejercicios, podemos pedirle que establezca la semilla para asegurar que los resultados que obtenga sean exactamente lo que esperamos que sean.

### Con y sin reemplazo

La función `sample` tiene un argumento que nos permite elegir más de un elemento de la urna. Sin embargo, por defecto, esta selección ocurre _sin reemplazo_: después de seleccionar una cuenta, no se vuelve a colocar en la bolsa. Observe lo que sucede cuando pedimos seleccionar al azar cinco cuentas:
...
`Error en sample.int (longitud (x), tamaño, reemplazo, problema):
no puede tomar una muestra más grande que la población cuando &#39;replace = FALSE&#39; &#39;

Sin embargo, el `sample` la función se puede usar directamente, sin el uso de `replicate`, para repetir el mismo experimento de elegir 1 de las 5 cuentas, continuamente, en las mismas condiciones. Para hacer esto, muestreamos _con reemplazo_: devuelva la cuenta a la urna después de seleccionarla.
Podemos decir `sample` hacer esto cambiando el `replace` argumento, que por defecto es `FALSE`, a `replace = TRUE`:
...
Decimos que dos eventos son independientes si el resultado de uno no afecta al otro. El ejemplo clásico es el lanzamiento de monedas. Cada vez que lanzamos una moneda justa, la probabilidad de ver caras es 1/2, independientemente de lo que hayan revelado los lanzamientos anteriores. Lo mismo es cierto cuando recogemos cuentas de una urna con reemplazo. En el ejemplo anterior, la probabilidad de rojo es 0.40 independientemente de los sorteos anteriores.

Muchos ejemplos de eventos que no son independientes provienen de juegos de cartas. Cuando repartimos la primera carta, la probabilidad de obtener un Rey es de 1/13 ya que hay trece posibilidades: As, Deuce, Tres, $\dots$, Ten, Jack, Queen, King y Ace. Ahora, si repartimos un Rey por la primera carta, y no la reemplazamos en el mazo, las probabilidades de que una segunda carta sea Rey es menor porque solo quedan tres Reyes: la probabilidad es 3 de 51. Estos eventos por lo tanto **no son independientes**: el primer resultado afectó al siguiente.

Para ver un caso extremo de eventos no independientes, considere nuestro ejemplo de dibujar cinco cuentas al azar **sin** reemplazo:
...
¿aún adivinarías azul? Por supuesto no. Ahora sabe que la probabilidad de rojo es 1 ya que la única cuenta que queda es roja. Los eventos no son independientes, por lo que las probabilidades cambian.
...
Esta es la forma matemática de decir: el hecho de que $B$ sucedió no afecta la probabilidad de $A$ sucediendo. De hecho, esto puede considerarse la definición matemática de independencia.
...
$$
Usemos Blackjack como ejemplo. En Blackjack, se le asignan dos cartas al azar. Después de ver lo que tiene, puede pedir más. El objetivo es acercarse a 21 que el distribuidor, sin pasar. Las cartas de cara valen 10 puntos y los ases valen 11 o 1 (tú eliges).

Entonces, en un juego de Blackjack, para calcular las posibilidades de obtener un 21 sacando un As y luego una carta de cara, calculamos la probabilidad de que el primero sea un As y multiplicamos por la probabilidad de sacar una carta de cara o un 10 dado que el primero fue un as: $1/13 \times 16/51 \approx 0.025$

La regla de multiplicación también se aplica a más de dos eventos. Podemos usar la inducción para expandirnos para más eventos:
...
Cuando tenemos eventos independientes, la regla de multiplicación se vuelve más simple:
...
Como ejemplo, imagine un caso judicial en el que se describió al sospechoso como que tenía bigote y barba. El acusado tiene bigote y barba y la fiscalía trae a un "experto" para que testifique que 1/10 hombres tienen barba y 1/5 tienen bigote, así que usando la regla de multiplicación concluimos que solo $1/10 \times 1/5$ o 0.02 tienen ambos.

¡Pero para multiplicar así necesitamos asumir independencia! Digamos que la probabilidad condicional de que un hombre tenga un bigote condicional en que tenga barba es .95. Entonces la probabilidad de cálculo correcta es mucho mayor: $1/10 \times 95/100 = 0.095$.
...
En nuestro primer ejemplo, imaginamos una urna con cinco cuentas. Como recordatorio, para calcular la distribución de probabilidad de un sorteo, simplemente enumeramos todas las posibilidades. Hubo 5 y entonces, para cada evento, contamos cuántas de estas posibilidades estaban asociadas con el evento. La probabilidad resultante de elegir una cuenta azul es 3/5 porque de los cinco resultados posibles, tres fueron azules.

Para casos más complicados, los cálculos no son tan sencillos. Por ejemplo, ¿cuál es la probabilidad de que si robo cinco cartas sin reemplazo, obtengo todas las cartas del mismo palo, lo que se conoce como "color" en el póker? En un curso de probabilidad discreta, aprende la teoría sobre cómo hacer estos cálculos. Aquí nos enfocamos en cómo usar el código R para calcular las respuestas.

Primero, construyamos una baraja de cartas. Para esto, usaremos el `expand.grid` y `paste` funciones Usamos `paste` para crear cadenas uniendo cadenas más pequeñas. Para hacer esto, tomamos el número y el palo de una tarjeta y creamos el nombre de la tarjeta de esta manera:
...
`paste` también funciona en pares de vectores que realizan la operación por elementos:

```{r}
paste(letters[1:5], as.character(1:5))
```

La función `expand.grid` nos da todas las combinaciones de entradas de dos vectores. Por ejemplo, si tiene pantalones azules y negros y camisas blancas, grises y a cuadros, todas sus combinaciones son:
...
Con el mazo construido, podemos verificar que la probabilidad de un Rey en la primera carta sea 1/13 calculando la proporción de posibles resultados que satisfagan nuestra condición:
...
Ahora, ¿qué tal la probabilidad condicional de que la segunda carta sea un Rey dado que la primera era un Rey? Anteriormente, dedujimos que si un Rey ya está fuera de la cubierta y quedan 51, entonces esta probabilidad es 3/51. Confirmemos enumerando todos los resultados posibles.

Para hacer esto, podemos usar el `permutations` función del paquete __gtools__. Para cualquier lista de tallas `n`, esta función calcula todas las diferentes combinaciones que podemos obtener cuando seleccionamos `r` artículos. Aquí están todas las formas en que podemos elegir dos números de una lista que consiste en `1,2,3`:
...
Observe que el orden importa aquí: 3,1 es diferente de 1,3. Además, tenga en cuenta que (1,1), (2,2) y (3,3) no aparecen porque una vez que elegimos un número, no puede volver a aparecer.
...
¿Qué tal si el orden no importa? Por ejemplo, en Blackjack si obtienes un As y una carta de cara en el primer sorteo, se llama _Natural 21_ y ganas automáticamente. Si quisiéramos calcular la probabilidad de que esto suceda, enumeraríamos las combinaciones, no las permutaciones, ya que el orden no importa.
...
Y luego verifica si una carta es un As y la otra una cara o un 10. En adelante, incluimos 10 cuando decimos _carta de cara_. Ahora necesitamos verificar ambas posibilidades:
