...
Las computadoras ofrecen una forma de realizar el experimento aleatorio sencillo descrito anteriormente: elija una bolita al azar de una bolsa que contiene tres bolitas azules y dos rojas. Los generadores de números aleatorios nos permiten imitar el proceso de selección al azar.
...
Gran parte de lo que estudian los estadísticos matemáticos y teóricos, que no cubrimos en este libro, se relaciona con proveer definiciones rigurosas de "prácticamente equivalente", [fix] así como estudiar qué tan cerca nos lleva un gran número de experimentos a lo que sucede en el límite. Más adelante en esta sección, ofrecemeos un acercamiento práctico para decidir qué es "lo suficientemente grande".
...
La función `sample` tiene un argumento que nos permite elegir más de un elemento de la urna. Sin embargo, por defecto, esta selección ocurre _sin reemplazo_: después de seleccionar una bolita, no se vuelve a colocar en la bolsa. Observe lo que sucede cuando pedimos seleccionar al azar cinco bolitas:
...
Esto da como resultado reordenamientos que siempre tienen tres bolitas azules y dos rojas. Si pedimos que se seleccionen seis bolitas, obtenemos un error:
...
Sin embargo, el `sample` la función se puede usar directamente, sin el uso de `replicate`, para repetir el mismo experimento de elegir 1 de las 5 bolitas, continuamente, en las mismas condiciones. Para hacer esto, muestreamos _con reemplazo_: devuelva la bolita a la urna después de seleccionarla.
...
Para ver un caso extremo de eventos no independientes, considere nuestro ejemplo de dibujar cinco bolitas al azar **sin** reemplazo:
...
Si tiene que adivinar el color de la primera bolita, predecirá el azul ya que el azul tiene un 60% de posibilidades. Pero si te muestro el resultado de los últimos cuatro resultados:

```{r}
x[2:5]
```

¿aún adivinarías azul? Por supuesto no. Ahora sabe que la probabilidad de rojo es 1 ya que la única bolita que queda es roja. Los eventos no son independientes, por lo que las probabilidades cambian.
