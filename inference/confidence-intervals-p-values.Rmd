...
Los intervalos de confianza son un concepto muy útil ampliamente utilizado por los analistas de datos. [fix check] Una versión de estos muy común proviene de la geometría `geom_smooth` de `ggplot`. Aquí tenemos un ejemplo usando un set de datos de temperatura disponible en R:
...
En nuestra competencia anterior, se les pidió que dieran un intervalo. Si el intervalo que indicaron incluye el $p$, obtienen la mitad del dinero que gastaron en su "encuesta" y pasan a la siguiente etapa de la competencia. Una forma de pasar a la segunda ronda es informar un intervalo muy grande. [fix] Por ejemplo, el intervalo $[0,1]$ está garantizado a siempre incluir a/siempre incluirá a $p$. Sin embargo, con un intervalo tan grande, no tenemos posibilidades de ganar la competencia. Del mismo modo, si ustedes son un pronosticadores de elecciones y predicen que la variabilidad será entre -100% y 100%, serán ridiculizados por decir lo obvio. [fix serio ok?] Incluso un intervalo más pequeño, como decir que la variabilidad será entre -10 y 10%, no se considerará serio.
...
3\. [fix "the final tally"] El conteo final para el voto popular fue Clinton 48.2% y Trump 46.1%. Agregue una columna, llámela `hit`, a la tabla anterior que indica si el intervalo de confianza incluía la proporción verdadera $p=0.482$ o no.
...
[fix see alternate] Los encuestadores no tienen éxito al proveer intervalos de confianza correctos, sino al predecir quién ganará [Los encuestadores no se consideran exitosos al proveer...]. Cuando tomamos un tamaño de muestra de 25 cuentas, el intervalo de confianza para la variabilidad:
...
Un problema con los resultados de nuestra encuesta es que, dado el tamaño de la muestra y el valor de $p$, tendríamos que sacrificar la probabilidad de [fix "an incorrect call"] una llamada/predicción incorrecta para crear un intervalo que no incluya 0.

Esto no significa que la elección está cerrada. Solo significa que tenemos un tamaño de muestra pequeño. En los libros de texto estadísticos esto se llama falta de _poder_. En el contexto de las encuestas, el _poder_ es la probabilidad de detectar [fix] variabilidades/diferencias spreads diferentes de 0.

[fix] Al aumentar el tamaño de nuestra muestra, disminuimos nuestro error estándar y, por lo tanto, tenemos muchas más posibilidades de detectar la dirección de la variabilidad/diferencia.


## valores p

[fix check spread trans ok] Los valores p son ubicuos en la literatura científica. Están relacionados con los intervalos de confianza, por lo que presentamos el concepto aquí.
...
Tengan en cuenta que existe una estrecha conexión entre los valores p y los intervalos de confianza. [fix] Si un intervalo de confianza de 95% de la variabilidad/diferencia no incluye 0, sabemos que el valor p debe ser menor que 0.05.
