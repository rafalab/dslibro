...
Para hacer nuestro mejor pronóstico, notamos que antes de que haya datos de encuestas disponibles, podemos usar fuentes de datos que no son datos de encuestas. Un enfoque popular es utilizar lo que los encuestadores llaman _fundamentos_, que se basan en características que históricamente parecen tener un efecto a favor o en contra del partido en poder como, por ejemplo, el estado de la economía. No usaremos estos aquí. En cambio, usaremos $\mu = 0$, que se interpreta como un modelo que no ofrece información sobre quién ganará. Para la desviación estándar, usaremos datos históricos recientes que muestran que el ganador del voto popular tiene una variabilidad promedio de aproximadamente 3.5%. Por lo tanto, establecemos $\tau = 0.035$.

Ahora podemos usar las fórmulas para la distribución posterior del parámetro $d$: la probabilidad de que $d>0$ dado los datos de la encuesta observada:
...
Para simular datos de un encuestador específico, ahora necesitamos dibujar un $h_i$ y luego agregar el $\varepsilon$s. Entonces, para un encuestador específico, suponemos que $\sigma_h$ es 0.025:
...
Tengan en cuenta que $h_i$ es común a todas las variabilidades observadas de un encuestador específico. Diferentes encuestadores tienen un $h_i$ diferente, lo que explica por qué cuando vemos los datos de los distintos encuestadores, podemos ver los diferentes grupos de puntos desplazarse hacia arriba y hacia abajo.
...
Aquí están las cinco estados con los resultados más cercanos según las encuestas:
...
6\. Para cada encuesta, calcule la diferencia entre la variabilidad que predijimos y la variabilidad observada. Añada una columna a la tabla `cis`. Entonces, añada otra columna llamada `hit` que es `TRUE` cuando los signos son los mismos. Sugerencia: use la función `sign`. Llame al objeto `resids`.


7\. Cree una gráfica como en el ejercicio 5, pero para la proporción de veces que los signos de la variabilidad estuvieron de acuerdo.
