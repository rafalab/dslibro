...
Para entender parte de la información incluida en este resumen, debemos recordar que los LSE son variables aleatorias. La estadística matemática nos da algunas ideas sobre la distribución de estas variables aleatorias.
...
Pueden ver que los estimadores de errores estándar informadas por la función `summary` están cerca de los errores estándar de la simulación:
...
Una vez que ajustemos nuestro modelo, podemos obtener predicciones de $Y$ usando las estimadores al modelo de regresión. Por ejemplo, si la altura del padre es $x$, entonces nuestra predicción $\hat{Y}$ para la altura del hijo será:
...
Nuestra tarea original era proveer un estimador y un intervalo de confianza para las estimaciones de pendiente de cada estrato. El paquete __broom__ hará esto bastante fácil.
...
8\. __Avanzado__. Escriba una función que tome R, HR y BB como argumentos y ajuste dos modelos lineales: `R ~ BB` y `R~BB+HR`. Luego use la función `do` para obtener el `BB` para ambos modelos para cada año desde 1961. Finalmente, grafíquelos como función de tiempo y compárelos.
