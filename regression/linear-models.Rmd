## [fix] Confusión/Variables confundidas/de confusión/Factor de confusión

Anteriormente, notamos una fuerte relación entre _Runs_ y BB. Si encontramos la línea de regresión para predecir carreras desde bases por bolas, obtendremos una pendiente de:
...
bb_slope
```
Entonces, ¿esto significa que si contratamos jugadores de bajo salario con muchos BB y así aumentamos por 2 el número de _walks_ por juego, nuestro equipo marcará `r round(bb_slope*2, 1)` más carreras por juego?
...
Además, observen que un sencillo lo lleva a primera base igual que un BB. Aquellos que saben de béisbol señalarán que con un sencillo, [fix] los corredores en base tienen una mejor oportunidad de anotar que con un BB. Entonces, ¿cómo puede un BB ser más predictivo de las carreras? La razón por la que esto sucede es por [fix]confunding/el factor de confusión. Aquí mostramos la correlación entre HR, BB y sencillos:
...
Resulta que los lanzadores, temerosos de los HR, a veces evitarán lanzar _strikes_ a los bateadores de HR. Como resultado, los bateadores de HR tienden a tener más BB y un equipo con muchos bateadores de HR también tendrá más BB. Aunque puede parecer que las BB causan carreras, en realidad son los HR los que causan la mayoría de estas carreras. [fix] Decimos que los BB están _confundidos_ con los HR. Sin embargo, ¿es posible que las BB todavía ayuden? Para averiguar, tenemos que [fix "adjust"] corregir.modificar/rectificar/ajustar para el efecto HR. La regresión también puede ayudar con esto.

### Comprender la confusión a través de la estratificación

Un primer enfoque es mantener los HR fijos en un cierto valor y luego examinar la relación entre BB y las carreras. Como lo hicimos cuando estratificamos a los padres redondeando a la pulgada más cercana, aquí podemos estratificar HR por juego a las diez más cercanas. Filtramos los estratos con pocos puntos para evitar estimaciones altamente variables:
...
Aunque nuestra comprensión de la aplicación nos dice que los HR causan BB pero no al revés, aún podemos verificar si la estratificación por BB hace que el efecto de BB disminuya. Para hacer esto, usamos el mismo código, excepto que intercambiamos HR y BB para obtener este gráfico:
...
Se reducen un poco, lo que es consistente con el hecho de que BB sí causan algunas carreras.
...
[fix check ok] De todos modos, parece que si estratificamos por HR, tenemos una distribución normal de dos variables para carreras versus BB. Del mismo modo, si estratificamos por BB, tenemos una distribución normal de dos variable aproximada para HR versus carreras.

### Regresión con múltiples variables/regresión lineal múltiple/regresión múltiple [fix]

Es un poco complejo calcular líneas de regresión para cada estrato. [fix "fitting" ok?] Básicamente, estamos ajustando modelos como este:
...
Si tomamos en cuenta la variabilidad aleatoria, las pendientes en los estratos no parecen cambiar mucho. Si estas pendientes son iguales, esto implica que $\beta_1(x_2)$ y $\beta_2(x_1)$ son constantes. [fix check] Esto a su vez implica que la expectativa de carreras condicionadas en HR y BB se puede escribir así:
...
Tengan en cuenta que si suponemos además que [fix check] el $\varepsilon$ tiene una distribución normal, entonces este modelo es exactamente el mismo que obtuvimos anteriormente [fix ending is wrong] asumiendo datos normales bivariados/distribución normal de dos variables. Una diferencia algo matizada es que en el primer enfoque asumimos que [fix] los datos eran bivariados normales/distribución normal de dos variables y que el modelo lineal se derivaba, no se suponía. En la práctica, los modelos lineales se suponen sin asumir necesariamente la normalidad: la distribución de $\varepsilon$s no se especifica. Sin embargo, si sus datos son bivariados normales, se cumple el modelo lineal anterior. Si sus datos no [fix] son bivariados normales, necesitarán justificar el modelo de otra forma.

### Interpretando modelos lineales

Una razón por la que los modelos lineales son populares es porque son interpretables. En el caso de los datos de Galton, podemos interpretar los datos de esta manera: debido a los genes heredados, la predicción de la altura del hijo crece por $\beta_1$ para cada pulgada que aumentamos la altura del padre $x$. Porque no todos los hijos con padres de estatura $x$ son de igual altura, necesitamos el término $\varepsilon$, lo que explica la variabilidad restante. Esta variabilidad restante incluye el efecto genético de la madre, los factores ambientales y otras aleatorias biológicas.

Dada la forma en que escribimos el modelo anterior, el intercepto $\beta_0$ no es muy interpretable, ya que es la altura prevista de un hijo con un padre sin altura. Debido a la regresión a la media, la predicción generalmente será un poco mayor que 0. Para hacer que el parámetro de pendiente sea más interpretable, podemos reescribir el modelo [fix "slightly as"] ligeramente como:
...
Para que los modelos lineales sean útiles, tenemos que estimar los $\beta$s desconocidos. El enfoque estándar en la ciencia es encontrar los valores que minimicen la distancia del modelo ajustado a los datos. La siguiente ecuación se llama la ecuación de mínimos cuadrados (LS o _least squares_ en inglés) y la veremos a menudo en este capítulo. Para los datos de Galton, escribiríamos:

$$
RSS = \sum_{i=1}^n \left\{ y_i - \left(\beta_0 + \beta_1 x_i \right)\right\}^2
$$

[fix choose both in ISI] Esta cantidad se denomina suma de errores cuadrados/suma de los cuadrados residual (RSS o _residual sum of squares_ en inglés). Una vez que encontremos los valores que minimizan el RSS, llamaremos a los valores las estimaciones de mínimos cuadrados (LSE 0 _least squares estimates_ en inglés) y los denotaremos con $\hat{\beta}_0$ y $\hat{\beta}_1$. Demostremos esto con el set de datos previamente definido:
...
Entonces, para cualquier par de valores, obtenemos un RSS. Aquí hay un gráfico de del RSS como función de $\beta_1$ cuando mantenemos el $\beta_0$ fijo en 25.
...
La prueba y el error no funcionarán en este caso. Podríamos buscar un mínimo dentro [fix fine grid] de una grilla fina de $\beta_0$ y $\beta_1$ valores, pero esto require mucho tiempo innecesariamente ya que podemos usar cálculo: tome las derivadas parciales, fíjelas en 0 y resuelva para $\beta_1$ y $\beta_2$. Por supuesto, si tenemos muchos parámetros, estas ecuaciones pueden volverse bastante complejas. Pero hay funciones en R que hacen estos cálculos por nosotros. Aprenderemos esto a continuación. Para aprender las matemáticas detrás de esto, puede consultar un libro sobre modelos lineales.

### La función `lm` 

En R, podemos obtener las estimaciones de mínimos cuadrados usando la función `lm`. [fix check] Para adaptarse al modelo:
...
Para ver cómo usamos el `lm` en un análisis más complejo, volvamos al ejemplo del béisbol. En un ejemplo anterior, estimamos líneas de regresión para predecir carreras para BB en diferentes estratos de HR. Primero construimos un marco de datos similar a este:
...
Primero, tenga en cuenta que si intentamos usar la función `lm` para obtener la pendiente estimada de esta manera:
...
Entonces, intentemos usar el `do` función para ajustar una línea de regresión a cada estrato de HR:
...
Al tratar de responder qué tan bien las BB predicen las carreras, la exploración de datos nos llevó a un modelo:
...
Nuestro modelo hace un buen trabajo, como lo demuestra el hecho de que los puntos de el gráfico observada frente a la predicción caen cerca de la línea de identidad.

Entonces, en lugar de usar el promedio de bateo, o solo el número de HR, como una medida de selección de jugadores, podemos usar nuestro modelo ajustado para formar una métrica que se relacione más directamente con la producción. Específicamente, para definir una métrica para el jugador A, imaginamos un equipo compuesto por jugadores como el jugador A y usamos nuestro modelo de regresión ajustado para predecir cuántas carreras produciría este equipo. La fórmula se vería así:
`r coefs$estimate[1]` +
...
Aquí rehacemos el gráfico sin jugadores que debutó antes de 1998. Usamos la función __lubridate__ `year`, introducido en la Sección \@ref(lubridate).
...
Vemos que todos estos jugadores tienen BB por encima del promedio y la mayoría tienen tasas de HR por encima del promedio, mientras que lo mismo no es cierto para sencillos. Aquí hay una tabla con estadísticas estandarizadas para todos los jugadores, de modo que, por ejemplo, los bateadores de HR por encima del promedio tienen valores superiores a 0.
...
3\. Tenga en cuenta que podemos reescribir OPS como un promedio ponderado de BB, sencillos, dobles, triples y HR. Sabemos que los pesos para dobles, triples y HR son 2, 3 y 4 veces mayores que los individuales. ¿Pero qué hay de BB? ¿Cuál es el peso para BB en relación con sencillos? Sugerencia: el peso de BB en relación con sencillos será una función de AB y PA.
