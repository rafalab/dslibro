...
El resultado no es exactamente lo que llamamos _tidy_ ya que cada observación está asociada con dos filas en vez de una. Queremos tener los valores de las dos variables, fertilidad y esperanza de vida, en dos columnas separadas. El primer desafío para lograr esto es separar la columna `key` en año y tipo de variable. Observen que las entradas en esta columna separan el año del nombre de la variable con una barra baja:
...
Reescriba la llamada a _gather_ usando un argumento que asegure que la columna del mes será numérica. Luego haz el gráfico.
...
Utilice la función `gather` para crear un _data frame_ `tmp` con una columna que contiene el tipo de observación `admitted` o `applicants`. Nombre a las nuevas columnas `key` y valor.


6\. Ahora tiene un objeto `tmp` con columnas `major`, `gender`, `key` y `value`. Tenga en cuenta que si combina `key` y `gender`, obtenemos los nombres de columna que queremos: `admitted_men`, `admitted_women`, `applicants_men` y `applicants_women`. Use la función `unite` para crear una nueva columna llamada `column_name`.
