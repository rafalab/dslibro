...
6\. Ahora tiene un objeto `tmp` con columnas `major`, `gender`, `key` y `value`. [fix check eng too] Tenga en cuenta que si combina `key` y `gender`, obtenemos los nombres de columna que queremos: `admitted_men`, `admitted_women`, `applicants_men` y `applicants_women`. Use la función `unite` para crear una nueva columna llamada `column_name`.
