...
Simplemente [fix do we need juntas? in eng same. how about concatenar en una?] concatenar estas dos tablas juntas no funcionará ya que el orden de los estados no es el mismo.
...
Ahora los [fix `NA`s?] NAs están en la columna de `tab_1`.
...
Si queremos mantener todas las filas y llenar las partes faltantes [fix `NA`s?] con NAs, podemos usar `full_join`. Pueden pensar en esto como una unión:
...
Queremos ver si la tendencia mensual está cambiando, por lo que eliminaremos los efectos del año y luego graficaremos los resultados. Primero calcularemos los promedios del año. Utilice `group_by` y `summarize` para calcular el CO2 promedio de cada año. Guarde en un objeto llamado `yearly_avg`.
