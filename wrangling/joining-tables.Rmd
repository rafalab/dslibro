...
La información que necesitamos para un análisis dado puede no estar solo en una tabla. Por ejemplo, cuando pronosticamos elecciones usamos la función `left_join` combinar la información de dos tablas. Aquí usamos un ejemplo más simple para ilustrar el desafío general de combinar tablas.
...
Las funciones _join_en el paquete__dplyr__ aseguran que las tablas se combinen para que las filas coincidentes estén juntas. Si conoce SQL, verá que el enfoque y la sintaxis son muy similares. La idea general es que uno necesita identificar una o más columnas que servirán para unir las dos tablas. Luego se devuelve una nueva tabla con la información combinada. Observe lo que sucede si unimos las dos tablas anteriores por estado usando `left_join` (eliminaremos el `others` columna y renombrar `electoral_votes` para que las tablas quepan en la página):
...
Los datos se han unido con éxito y ahora podemos, por ejemplo, hacer un diagrama para explorar la relación:
...
Vemos que la relación es casi lineal con aproximadamente 2 votos electorales por cada millón de personas, pero con estados muy pequeños que obtienen proporciones más altas.


En la práctica, no siempre ocurre que cada fila de una tabla tenga una fila coincidente en la otra. Por esta razón, tenemos varias versiones de join. Para ilustrar este desafío, tomaremos subconjuntos de las tablas anteriores. Creamos las tablas `tab1` y `tab2` para que tengan algunos estados en común pero no todos:
...
Supongamos que queremos una mesa como `tab_1`, pero agregando votos electorales a cualquier estado que tengamos disponible. Para esto, usamos `left_join` con `tab_1` como el primer argumento Especificamos qué columna usar para que coincida con `by` argumento.

```{r}
left_join(tab_1, tab_2, by = "state")
```

Tenga en cuenta que `NA` s se agregan a los dos estados que no aparecen en `tab_2`. Además, observe que esta función, así como todas las otras uniones, pueden recibir los primeros argumentos a través de la tubería:
...
Ahora los NA están en la columna de `tab_1`.

### Unir internamente

Si queremos mantener solo las filas que tienen información en ambas tablas, usamos `inner_join`. Puedes pensar en esto como una intersección:
...
Si queremos mantener todas las filas y llenar las partes faltantes con NA, podemos usar `full_join`. Puedes pensar en esto como una unión:
...
Los `semi_join` la función nos permite mantener la parte de la primera tabla para la que tenemos información en la segunda. No agrega las columnas del segundo:
...
La función `anti_join` es lo contrario de `semi_join`. Mantiene los elementos de la primera tabla para los que no hay información en la segunda:
...
Podemos ver los nombres de las columnas `nameFirst` y `nameLast`. Utilizar el `left_join` función para crear una tabla de los mejores bateadores de jonrones. La mesa debe tener `playerID`, nombre, apellido y número de jonrones (HR). Reescribe el objeto `top` con esta nueva mesa
