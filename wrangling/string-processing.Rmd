...
| `str_replace` | Reemplazar | [fix sounds odd] Reemplaza la primera parte de una cadena que coincida con un patrón con otro. | |
...
| `str_sort` | Manipular | Pon el vector en orden alfabético.  |`sort` |
...
De hecho, escapar con la barra diagonal inversa ofrece una forma de definir la [fix ok? "while still using"] cadena mientras y a la vez seguir utilizando las comillas dobles para definir cadenas:

```{r}
s <- "10\""
```

En R, la función `cat` nos deja ver como se ve la cadena:
...
A lo largo de esta sección, verán que creamos cadenas para probar nuestra expresión regular. Para hacer esto, definimos patrones que sabemos que deberían coincidir y también patrones que sabemos que no deberían coincidir. Los llamaremos `yes` y `no`, respectivamente. Esto nos permite verificar los dos tipos de errores: no coincidir o coincidir incorrectamente.
...
[fix 'matches' ok?] y `str_view_all` nos muestra todos las coincidencias, entonces `3'2` tiene dos coincidencias y `5'10` tiene tres.
...
Sin embargo, es importante saber que en _regex_ todo es un carácter; no hay números. Entonces `4` es el carácter `4` no el número cuatro. Noten, por ejemplo, que `[1-20]` **no** significa 1 a 20, significa los caracteres 1 a 2 o el carácter 0. Entonces `[1-20]` simplemente significa la clase de caracteres compuesta por 0, 1 y 2.
...
Para la parte de pulgadas, podemos tener uno o dos dígitos. Esto se puede especificar en _regex_ con _cuantificadores_ (_quantifiers_ en inglés). Esto se hace siguiendo [fix con llaves = curly brackets] el patrón con llaves que contienen la cantidad de veces que se puede repetir la entrada anterior. Usemos un ejemplo para ilustrar. El patrón para uno o dos dígitos es:
...
Para evitar cambiar patrones como `70.2`, requeriremos que el primer dígito esté entre 4 y 7 `[4-7]` y [fix 'none or more digits' - is that ok?] que el segundo sea ninguno o más dígitos `\\d*`. Comencemos definiendo un patrón sencillo que coincida con esto:
