...
| `str_replace` | Reemplazar | Reemplaza la primera parte de una cadena que coincide con un patrón con otro patrón. | |
| `str_replace_all` | Reemplazar | Reemplaza todas las partes de una cadena que coincida con un patrón con otro patrón. | `gsub` |
...
[fix 'matches' ok?] y `str_view_all` nos muestra todos los patrones que corresponden, entonces `3'2` tiene dos coincidencias y `5'10` tiene tres.
...
Los _grupos_ son un aspecto poderoso de la expresión regular que permite la extracción de valores. Los grupos se definen usando paréntesis. No afectan la coincidencia de patrones. En cambio, permiten que las herramientas identifiquen partes específicas del patrón para que podamos extraerlas.

Queremos cambiar las alturas escritas como `5.6` a `5'6`.

Para evitar cambiar patrones como `70.2`, requeriremos que el primer dígito esté entre 4 y 7 `[4-7]` y que el segundo sea ninguno o uno o más dígitos `\\d*`. Comencemos definiendo un patrón sencillo que coincida con esto:
