...
Uno de los desafíos más comunes del _wrangling_ de datos consiste en extraer datos numéricos contenidos en cadenas de caracteres y convertirlos en las representaciones numéricas requeridas para hacer gráficos, calcular resúmenes o ajustar modelos en R. También, es común tener que procesar texto no organizado en [fix check] nombres de variables significativas o variables categóricas. Muchos de los desafíos de procesamiento de cadenas que enfrentan los científicos de datos son únicos y a menudo inesperados. Por lo tanto, es bastante ambicioso escribir una sección completa sobre este tema. Aquí usamos una serie de estudios de casos que nos ayudan a demostrar cómo el procesamiento de cadenas es un paso necesario para muchos desafíos de disputas de datos. Específicamente, [fix check for clarity] describimos el proceso de convertir los datos _raw_ originales aún no mostrados de los que extrajimos el ejemplo `murders`, `heights` y `research_funding_rates` en los _data frames_ que hemos estudiado en este libro.
...
La mayoría de los ejemplos provendrán del segundo estudio de caso que trata sobre las alturas autoreportadas por los estudiantes y la mayor parte del capítulo está dedicado al aprendizaje de expresiones regulares (_regular expressions_ _regex_ en inglés) y funciones en el paquete __stringr__.
...
| `str_locate_all` | Localizar | Devuelve la posición de todas las apariciones de patrón en una cadena. | `gregexpr` |
| `str_view`  | Localizar | Muestra la primera parte de la cadena que coincide con el patrón. |  |
| `str_view_all` | Localizar | Muéstrame todas las partes de la cadena que coinciden con el patrón. |
| `str_extract` | Extracto | Extraiga la primera parte de la cadena que coincida con el patrón. |  |
`str_extract_all` | Extracto | Extraiga todas las partes de la cadena que coincidan con el patrón. |  |
| `str_match` | Extracto | Extraiga la primera parte de la cadena que coincida con los grupos y los patrones definidos por los grupos.|
| `str_match_all` | Extracto | Extraiga todas las partes de la cadena que coincidan con los grupos y los patrones definidos por los grupos. | |
| `str_sub`| Extracto | Extraer una subcadena.| `substring` |
| `str_split`| Extracto | Dividir una cadena en una lista con partes separadas por patrón.| `strsplit`|
| `str_split_fixed`| Extracto | Dividir una cadena en una matriz con partes separadas por patrón. | `strsplit` con `fixed = TRUE`|
| `str_count` | Describa | Cuente el número de veces que aparece un patrón en una cadena. | |
| `str_length` | Describa | Número de caracteres en la cadena. | `nchar` |
| `str_replace` | Reemplazar | Reemplace la primera parte de una cadena que coincida con un patrón con otro. | |
| `str_replace_all` | Reemplazar | Reemplace todas las partes de una cadena que coincida con un patrón con otro. | `gsub` |
...
| `str_to_title` | Reemplazar | Cambie el primer carácter a superior y descanse a inferior. | |
| `str_replace_na` | Reemplazar | Reemplaza todo `NA` s a un nuevo valor. | |
| `str_trim` | Reemplazar | Elimine el espacio en blanco desde el inicio y el final de la cadena. | |
| `str_c` | Manipular | Unir múltiples cadenas. | `paste0` |
| `str_conv` | Manipular | Cambiar la codificación de la cadena. |
| `str_sort` | Manipular | Ordenar el vector en orden alfabético. `sort` |
| `str_order` | Manipular | Índice necesario para ordenar el vector en orden alfabético. | `order` |
| `str_trunc` | Manipular | Truncar una cadena a un tamaño fijo. | |
| `str_pad` | Manipular | Agregue espacio en blanco a la cadena para que tenga un tamaño fijo. | |
| `str_dup` | Manipular | Repite una cuerda. | `rep` luego `paste` |
| `str_wrap` | Manipular | Envuelva cosas en párrafos formateados. | |
| `str_interp` | Manipular | Interpolación de cuerdas. | `sprintf` |


## Estudio de caso 1: datos de asesinatos en EE. UU.


En esta sección presentamos algunos de los desafíos de procesamiento de cadenas más sencillas con los siguientes  sets de datos como ejemplo:
...
[fix ok?] Forzar una conversión como siempre no funciona aquí:
...
Entonces podemos usar la función `str_replace_all` para eliminarlos:
...
Este caso es relativamente sencillo en comparación con los desafíos de procesamiento de cadenas que normalmente enfrentamos en la ciencia de datos. El siguiente ejemplo es bastante complejo y ofrece varios desafíos que nos permitirán aprender muchas técnicas de procesamiento de cadenas.
...
La función `extract` del paquete __tidyr__ nos permite usar grupos _regex_ para extraer los valores deseados. [fix eng off] Aquí está el equivalente al código anterior usando `separate` pero usando `extract`:
...
Esta función [fix 'read-in']  lee/importa los datos línea por línea para crear un vector de cadenas. En este caso, una cadena para cada fila en la hoja de cálculo. Las primeras seis líneas son:
...
Sin embargo, debido a que esta es una tarea tan común, __purrr__ provee un acceso directo. Si el segundo argumento recibe [fix check from here to end] un número? entero en lugar de una función, supondrá que queremos esa entrada. Entonces, el código anterior se puede escribir de manera más eficiente de esta manera:
