...
(Página web cortesía de Wikipedia^[https://en.wikipedia.org/w/index.php?title=Gun_violence_in_the_United_States_by_state&amp;direction=prev&amp;oldid=810166167]. Licencia CC-BY-SA-3.0^[https://en.wikipedia .org/ wiki/ Wikipedia: Text_of_Creative_Commons_Attribution-ShareAlike_3.0_Unported_License]. [fix quitar?] Captura de pantalla de parte de la página.)

Desafortunadamente, no hay un enlace a un archivo de datos. Para hacer el _data frame_ que se carga cuando escribimos `data(murders)`, tuvimos que hacer algo de _desguace web_.

_Extracción de la web_ (_web scraping_ o _web harvesting_ en inglés) es el término que usamos para describir el proceso de extracción de datos de un sitio web. La razón por la que podemos hacer esto es porque la información utilizada por un navegador para representar páginas web se recibe como un archivo de texto de un servidor. El texto es un código escrito en lenguaje de marcado de hipertexto (HTML). Cada navegador tiene una manera de mostrar el [fix check] código fuente html de una página, cada uno diferente. En Chrome, pueden usar Control-U en una PC y comando + alt + U en una Mac. Verán algo como esto:
...
Esto claramente no es un set de datos ordenado, ni siquiera un _data frame_. En el código anterior, definitivamente puede ver un patrón y escribir código para extraer solo los datos es muy factible. De hecho, __rvest__ incluye una función solo para convertir tablas HTML en _data frames_:
...
Si el contenido de este objeto es una tabla html, podemos usar el `html_table` función para convertirlo en un _data frame_. Utilizar el `html_table` función para convertir la octava entrada de `nodes` en una mesa
