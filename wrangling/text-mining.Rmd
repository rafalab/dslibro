...
Utilizaremos las siguientes [fix "libraries"?] bibliotecas:
...
unnest_tokens(word, text, token = "_tweets_") %>%
pull(word)
```
...
Ahora que tenemos todas nuestras palabras en una tabla, [fix eng too] junto con información sobre qué dispositivo se usó para componer el tuit del que provienen, podemos comenzar a explorar qué palabras son más comunes al comparar Android con iPhone.

Para cada palabra, queremos saber si es más probable que provenga de un tuit de Android o un tuit de iPhone. En la sección \@ref(association-tests) introdujimos el _odds ratio_ o _riesgo relativo_ como un resumen estadístico útil para cuantificar estas diferencias. Para cada dispositivo y una palabra dada, llamémosla `y`, [fix rest of para] calculamos las probabilidades o la razón/riesgo relativo entre la proporción de palabras que son `y` y no `y` y calculamos la razón de esas probabilidades. Aquí tendremos muchas proporciones que son 0, así que usamos la corrección 0.5 descrita en la Sección [fix not NA should be 15.10 fix code] NA.
...
[fix check] Aquí están los riesgos relativos más altos para Android:

```{r}
android_iphone_or %>% arrange(desc(or))
```

y la [fix check] parte superior para iPhone:
```{r}
...
6\. Elimine [fix "stop" words] las palabras de parada y los números del objeto `words`. Sugerencia: use el `anti_join`.


7\. Ahora use el léxico `AFINN` para asignar un valor de sentimiento a cada palabra.


8\. Haga una gráfica de puntuación de sentimiento versus ubicación en el libro y agregue [fix smoother] un suavizador.
