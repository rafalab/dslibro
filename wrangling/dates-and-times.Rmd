...
Los convierte en días desde la época. La función `as.Date` puede convertir un carácter en una fecha. Entonces, para ver que la época es el día 0, podemos escribir:
...
La función `mdy` asume que la primera entrada es el mes, luego el día, luego el año:

```{r}
mdy(x)
```

El paquete _lubridate_ provee una función para cada posibilidad:
```{r}
ydm(x)
myd(x)
dmy(x)
...
El paquete __lubridate__ también es útil para lidiar con los tiempos. En R base, puede obtener la hora actual escribiendo `Sys.time()`. El paquete __lubridate__ ofrece una función un poco más avanzada, `now`, que le permite definir la zona horaria:
...
Puede ver todas las zonas horarias disponibles con la función `OlsonNames()`.
...
La función `make_date` se puede utilizar para crear rápidamente un objeto de fecha. Toma tres argumentos: año, mes, día, hora, minuto, segundos y zona horaria predeterminada a los valores de época en la hora UTC. Entonces cree un objeto de fecha que represente, por ejemplo, el 6 de julio de 2019, escribimos:
